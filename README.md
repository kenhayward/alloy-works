...
> carries
> that formatting, and footnotes and a table's note are written and published. A cross-reference is
> made in the editor and shows what it will print, but is not yet published, and nothing holds an
> equation.
...
bar needs themes, templates, assets and tables as well as an editor. **The first pieces of it are built** - tenancy and sign-in, access control, components with immutable versions, a component's paragraphs edited, formatted and arranged into lists under a lock, documents whose outlines you restructure and whose sections are numbered, and a document published as a laid-out, tagged PDF that carries that formatting, those lists, its tables and its figures - on the scaffolding under them. Published cross-references, equations, themes, templates, search and Word are not, and the table below and [`docs/features.md`](docs/features.md) say exactly what is.
...
| Cross-references             | A reference to a section, a figure, a table or a footnote, from the toolbar or `Ctrl+Alt+X`, chosen in a dialog and shown as it will print, or as broken when its target has gone; kept through an undo and a cut and paste; not yet published                                                                                                                                                                                             |
