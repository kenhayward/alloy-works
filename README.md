...
| Figures                      | Make a figure from a PNG or JPEG on the toolbar, describing the image or marking it decorative as it is uploaded; caption it, change how it is described, replace its image or delete it from a Figure panel. Publishing one is refused until a later slice                                                                                                 |
...
| Publishing a document        | Publish a document as a tagged PDF of its outline, its formatted paragraphs, its lists, its quotations, its preformatted text and its tables, laid out with a cover, a contents, a list of tables, running heads and feet and pages numbered per part, marked not approved on every page, kept, listed beneath the outline and downloaded from its own page |
