...
> carries
> that formatting, and footnotes, a table's note and cross-references are written and published.
> Equations are written in the editor, and not yet published.
...
bar needs themes, templates, assets and tables as well as an editor. **The first pieces of it are built** - tenancy and sign-in, access control, components with immutable versions, a component's paragraphs edited, formatted and arranged into lists under a lock, documents whose outlines you restructure and whose sections are numbered, and a document published as a laid-out, tagged PDF that carries that formatting, those lists, its tables, its figures, its footnotes and its cross-references - on the scaffolding under them. Published equations, themes, templates, search and Word are not, and the table below and [`docs/features.md`](docs/features.md) say exactly what is.
...
| Cross-references             | A reference to a section, a figure, a table or a footnote, from the toolbar or `Ctrl+Alt+X`, chosen in a dialog and shown as it will print, or as broken when its target has gone; kept through an undo and a cut and paste; published as its number, its title, both, its page, or above or below in the layout's words, a link in running text                                                                                                       |
| Equations                    | An equation typed as LaTeX in a dialog, from the toolbar or `Ctrl+Shift+E`, and drawn as mathematics as you type, inline in any text or as a block of its own, numbered or not, with the words a screen reader says written for you in the component's language and yours to change; not yet published                                                                                                                                                 |
