// The publication template, version 5 (docs/design/publishing.md). It reads the published
// document under a layout whose blocks may be lists, quotations and preformatted text,
// `publishing/5`, from data.json as values and evaluates nothing: no content reaches Typst as source
// (ADR-0013, PUB-062). A version is immutable - an edit is a new directory, and a test holds each
// version's hash. It is version 4 with two branches of `block-of` and the monospace rules, and
// nothing else: template 4 and the publications made with it are a record, never migrated.
#let doc = json("data.json")
#assert(doc.schema == "publishing/5", message: "data.json is not publishing/5")
