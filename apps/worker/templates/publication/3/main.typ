// The publication template, version 3 (docs/design/publishing.md). It reads the published
// document under a layout whose runs carry their marks, `publishing/3`, from data.json as values and
// evaluates nothing: no content reaches Typst as source (ADR-0013, PUB-062). A version is immutable -
// an edit is a new directory, and a test holds each version's hash. It is version 2 with `run`, and
// nothing else: template 2 and the publications made with it are a record, never migrated.
#let doc = json("data.json")
#assert(doc.schema == "publishing/3", message: "data.json is not publishing/3")
