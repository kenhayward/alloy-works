// The publication template, version 13 (docs/design/publishing.md). It reads the published
// document whose tables and images are set from their styles, `publishing/13`, from data.json as
// values and evaluates nothing: no content reaches Typst as source (ADR-0013, PUB-062). A version is
// immutable - an edit is a new directory, and a test holds each version's hash. It is version 12 -
// the face, size, weight, posture and colour of all its text, a paragraph's fill, padding, alignment
// and indents, its line spacing and the space between blocks, and the size of a subscript or a
// superscript, read from the document's `theme` by the rules ADR-0014 measured (themes 1, ruling R7)
// - with a table's rules, fills, cells' inset, header weight, header repetition, rows kept whole and
// continuation label read from its table style, a figure's placement and alignment from its image
// style, and contextual spacing between two paragraphs of one style that both ask for it (themes 2,
// ruling R6), none of it written as a literal here: `template.test.ts` reads this file and fails on a
// typographic literal outside the short list it allows. Template 12 and the publications made with it
// are a record, never migrated.
//
...
// M3). A list's and an enumeration's indent and body indent; a definition list's hanging indent and
// its separator; the contents' leader of dots and an entry's indent at each depth; the underline's
// offset and thickness; how far a subscript or a superscript is lowered or raised (the face's own OS/2
// tables); and a footnote's separator, clearance and indent at the foot of its page. No property of
// themes 1 or 2 names any of them. A table's inset and rules, which template 12 left to the engine,
// are the table style's here.
#let doc = json("data.json")
#assert(doc.schema == "publishing/13", message: "data.json is not publishing/13")
...
//
// CONTEXTUAL SPACING (themes 2, ruling R1), as Word's `w:contextualSpacing`: between two blocks set in
// ONE style - the same identifier, not two styles that happen to state the same values - which both
...
// So the space around a quotation is its paragraphs' own, spaced as any two blocks are - and set off
// again since themes 2 (ruling R3): the default quotation style's space before and after, and its
// attribution's, are template 11's, and it asks for contextual spacing, so its own paragraphs stand a
// line apart rather than set off from each other too (`between`).
...
// worked out from its image style - 1.2 ems of the style it stands in under the default theme, and
// never wider than the room it stands in - which places it `inline` and nowhere else. Its
...
#let block-of(b, at, before: none) = {
  if type(b) == array {
    ...
    // The table style it is set in (themes 2, ruling R6): its rules, fills, cells' inset, header
    // weights and how it breaks across pages, each as the theme states it and none decided here.
    let t = theme.tables.at(b.style)
    // Named apart from the engine's own `stroke`, `fill` and `label`, which this branch never shadows.
    let rule(s) = if s == none { none } else { (thickness: pt(s.thickness), paint: colour(s.paint)) }
    // A continued table's label stands in a first header row of its own, `lead` rows above the
    // table's own; every row index below counts from the table's own first row.
    let lead = if t.continuationLabel { 1 } else { 0 }
    ...
    let cell-of(c, kept) = {
      ...
      let weight = if c.scope == "column" {
        ...
        t.headerColumn.weight
      } else if c.scope == "both" {
        if t.headerRow.weight != none { t.headerRow.weight } else { t.headerColumn.weight }
      } else {
        none
      }
      let body = if weight == none { body } else {
    ...
    // it, so the two cells either side of a line agree on it and neither overrides the other; the
    // table's foot and its far side are lines of their own after the cells. The outer rule frames the
    // table - and, measured, frames each page's part of a table that breaks, the engine drawing the
    // table's first and last lines at every page's edge of it. The header column's rule stands after
    // its last column; the header row's, below its last row, is a line of the header's own, so that it
    // repeats beneath the header on every page the header does (measured: drawn by the cells, it stood
    // beneath the header on the first page alone). Between the label's row and the header there is no
    // rule: on the first page the two lines are one, the table's top.
    let across(y) = if y == 0 {
      rule(t.strokes.outer)
    } else if lead == 1 and y == 1 {
      none
    } else {
      rule(t.strokes.horizontal)
    }
    let down(x) = if x == 0 {
      rule(t.strokes.outer)
    } else if x == b.headerColumns and t.headerColumn.stroke != none {
      rule(t.headerColumn.stroke)
    } else {
      rule(t.strokes.vertical)
    }
    // The fills (STY-076): the header row's behind its cells, the corner included; the header
    // column's behind the rest of its cells; and the band behind every other body row, counting from
    // the first body row, which is banded. The label's row has none. Where a cell spans, the fill is
    // the one of the row and column it starts in.
    let filled(x, y) = {
      let row = y - lead
      if row < 0 {
        none
      } else if row < b.headerRows and t.headerRow.fill != none {
        colour(t.headerRow.fill)
      } else if x < b.headerColumns and t.headerColumn.fill != none {
        colour(t.headerColumn.fill)
      } else if row >= b.headerRows and t.band != none and calc.even(row - b.headerRows) {
        colour(t.band)
      } else {
        none
      }
    }
    let header-rule = if b.headerRows > 0 and t.headerRow.stroke != none {
      (table.hline(stroke: rule(t.headerRow.stroke)),)
    } else {
      ()
    }
    ...
    // style says - a header of a second level, measured to repeat, or not, apart from the first.
    let headers = if t.continuationLabel {
      (table.header(level: 1, repeat: true, continued),)
      if heading.len() > 0 { (table.header(level: 2, repeat: t.repeatHeader, ..heading, ..header-rule),) }
    } else if heading.len() > 0 {
      (table.header(repeat: t.repeatHeader, ..heading, ..header-rule),)
    } else {
      ()
    }
    ...
    let set-table(rest) = labelled(b.anchor, figure(
      ...
      table(
        ...
        inset: pt(t.inset),
        stroke: (x, y) => (top: across(y), left: down(x)),
        fill: filled,
        ..headers,
        ..rest,
        ..if t.strokes.outer == none { () } else {
          (table.hline(stroke: rule(t.strokes.outer)), table.vline(x: b.columns, stroke: rule(t.strokes.outer)))
        },
      ),
    ))
    ...
    //
    // Placed and aligned as its image style says (STY-018, TH-J): as a block in the flow of the text,
    // or floated - `figure`'s own `placement: auto`, which the engine sets as a band at the head or the
    // foot of the page, whichever is nearer where the figure stands, text never running beside it: the
    // only float it has, measured. Either way the image stands at the start, the centre or the end of
    // the measure, in a block as wide as the measure, since a figure is otherwise as wide as the wider
    // of its image and its caption and an alignment inside it would move the image by nothing; the
    // caption is aligned across the measure too, by its own style.
    let caption = role("caption")
    labelled(b.anchor, figure(
      kind: image,
      numbering: none,
      placement: if b.placement == "float" { auto } else { none },
      gap: pt(caption.spaceBefore + caption.leading),
      caption: {
        if b.label != none { b.label + " " }
        b.caption.map(run).join()
      },
      block(width: 100%, align(alignment(b.alignment), shown)),
    ))
  } else if b.type == "equation" {
...
#let listed(title, entry, body) = {
  ...
  show outline.entry: set block(spacing: pt(between(entry, entry)))
