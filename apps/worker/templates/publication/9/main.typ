// The publication template, version 9 (docs/design/publishing.md). It reads the published
// document under a layout whose runs may be images or footnotes and whose blocks may be lists,
...
#assert(doc.schema == "publishing/9", message: "data.json is not publishing/9")
...
#let base-run(r) = if "image" in r { image-run(r.image) } else { text-run(r) }
