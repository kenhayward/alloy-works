// The publication template, version 7 (docs/design/publishing.md). It reads the published
// document under a layout whose blocks may be lists, quotations, preformatted text, tables and
// figures, with its generated lists after the contents, `publishing/7`, from data.json as values and
// evaluates nothing: no content reaches Typst as source (ADR-0013, PUB-062). A version is immutable -
// an edit is a new directory, and a test holds each version's hash. It is version 6 with a figure
// branch of `block-of` and figures among the lists, and nothing else: template 6 and the publications
// made with it are a record, never migrated.
#let doc = json("data.json")
#assert(doc.schema == "publishing/7", message: "data.json is not publishing/7")
