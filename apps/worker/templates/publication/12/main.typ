...
// directory, and a test holds each version's hash - but this one is not yet published: until themes
// 1's task 4 sets every face, size, colour and space from the document's `theme`, it is version 11
// with nothing changed but these lines and the schema it asserts, and `publishing/12` is
// `publishing/11` with that `theme` and a paragraph's `style` beside what version 11 reads.
