// The publication template, version 6 (docs/design/publishing.md). It reads the published
// document under a layout whose blocks may be lists, quotations, preformatted text and tables, with
// its generated lists after the contents, `publishing/6`, from data.json as values and evaluates
// nothing: no content reaches Typst as source (ADR-0013, PUB-062). A version is immutable - an edit is
// a new directory, and a test holds each version's hash. It is version 5 with a table branch of
// `block-of`, the table rules and the lists after the contents, and nothing else: template 5 and the
// publications made with it are a record, never migrated.
#let doc = json("data.json")
#assert(doc.schema == "publishing/6", message: "data.json is not publishing/6")
...
#let kind-of(sequence) = if sequence == "table" { table } else { panic("unknown list: " + sequence) }
