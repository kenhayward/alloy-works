// The publication template, version 10 (docs/design/publishing.md). It reads the published
// document under a layout whose runs may be images, footnotes or cross-references and whose blocks may
// be lists, quotations, preformatted text, tables with their notes, figures and the empty markers a
// reference can name, with its generated lists after the contents, `publishing/10`, from data.json as
// values and evaluates nothing: no content reaches Typst as source (ADR-0013, PUB-062). A version is
// immutable - an edit is a new directory, and a test holds each version's hash. It is version 9 with a
// reference branch of `run`, a label on every element a reference names, and a marker block, and
// nothing else: template 9 and the publications made with it are a record, never migrated.
#let doc = json("data.json")
#assert(doc.schema == "publishing/10", message: "data.json is not publishing/10")
...
// names the node, and no label where none does.
#let node(n) = {
  let own = {
