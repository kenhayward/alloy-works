 ...
 * `publishing/4`, the same again with a block that may be a list. Versions 2 and 3 are kept although
 * `assemble` makes neither schema any more: they are what the publications made before a run carried
 * its marks, and before a block could be a list, were compiled with, and a published version is a
 * record.
 */
 ...
 * answer F); every request since is made under a layout, and publishes with template 4. No row names
 * template 2 or template 3, because nothing makes a document of either schema to hand them.
 *
