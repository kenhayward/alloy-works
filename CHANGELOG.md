...
- **Sixteen more requirements are now shown by a test**, each one watched fail first by breaking
  the code it covers. They were built already, but no test named them. Among them: pasting from
  Markdown or a web page keeps every structure a component can hold, and says what it changed; an
  underline is drawn in the PDF; undo stops at a version you saved; each mark goes on and comes off
...
- **Footnotes now reach Word.** A document holding footnotes, which Word publishing refused until
...
- **Cross-references now reach Word**, each one a field Word updates rather than words typed in:
  ...
  right as it opens, but for pages, which Word fills in from its own pages when the reader agrees to
...
- **Publishing to Word refuses, by name, a cross-reference Word would print otherwise than the PDF**,
  ...
  The default layout's numbering is inside what Word counts.
