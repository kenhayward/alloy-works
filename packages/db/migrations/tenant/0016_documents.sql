...
-- The event is artifact_version_component_type_recorded's check (0008, deferrable initially
-- deferred), and it passes: the starter type's row names no component type. An upgrade from 0015
-- never meets it, because 0015 committed in an earlier run - which is why only a fresh environment
-- shows it (document-migration.test.ts runs both).
