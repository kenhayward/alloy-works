  ...
  it(
    ...
    () => {
      ...
      const known = {
        ...
        q: [{ block: 'e2', sequence: 'equation', numbered: true }],
        r: [
          { block: 'ea1', sequence: 'equation', numbered: true },
          { block: 'ta1', sequence: 'table', numbered: true },
          { block: 'fa1', sequence: 'footnote', numbered: true },
        ],
        s: [{ block: 'fb1', sequence: 'footnote', numbered: true }],
      };
      const numbered = table(outline, known);
      // Body equations are continuous: an appendix later in the document restarts nothing before it.
      expect(labels(numbered.entries, 'equation')).toEqual([
        'Equation 1',
        'Equation 2',
        'Equation A.1',
      ]);
      expect(labels(numbered.entries, 'table')).toEqual(['Table A.1']);
      // Footnotes carry no label in the default scheme, and continue across both appendices.
      expect(labels(numbered.entries, 'footnote')).toEqual(['1', '2']);
    },
  );
