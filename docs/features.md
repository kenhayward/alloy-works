...
- **An interface in one theme.** Every screen sits under a dark header band with:
  ...
  their counts, which hides to a rail, with New component in a dialog and a menu on each row to open it, copy its link or manage its access; there is no search, sort, or filter by type, language or
