...
| Refusing a mark identifier that is not in NFC                                                      | Found by the final re-review: a mark with a decomposed identifier is accepted and `canonicalJson` folds it to its composed spelling, so two fragments of one annotation spelled the two ways resolve as two annotations but hash as one (CNT-004, CNT-005). It predates this plan, which applied the rule to block, footnote and cross-reference identifiers and to targets only                                                                                                                                      | **The plan that first authors a mark with an identifier**                    |
