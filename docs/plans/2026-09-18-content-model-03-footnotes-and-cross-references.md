...
#73 as **STR-062**, reworded (decision C).
...
10. **Inside a footnote, the schema's defaults are never applied.** A footnote's `content` is parsed
    ...
    two strings and two digests. Nothing becomes unreadable - a later fix applies the defaults on read
    - so it is **named and not fixed here**; see [What this plan deliberately leaves undone](#what-this-plan-deliberately-leaves-undone).
...
| 10  | A footnote's `content`                        | `inlineNodeSchema` leaves it `z.array(z.unknown())`; the walk parses it with `footnoteContentSchema` and now refuses an image or a footnote in its paragraphs                 | **The loose one in the schema, tightened only by the walk** - which is why both homes must run the walk, and both do. Finding 10: the walk's parsed copy, with defaults, is discarded | **Bounded at one footnote**: no footnote inside one, so the walk descends once and stops | Nothing                                                                                                                                                                                                                                       |
...
in admission (finding 11), and a footnote's defaults are still not applied (finding 10). Neither makes
anything stored unreadable later, which is the test this table applies.
...
| Applying a footnote's defaults (`style`, `marks`) as the parse applies them elsewhere         | Finding 10: two spellings, two digests, and nothing unreadable. Recommended as its own issue                             | **A small content-model fix**                        |
| Bounding a block tree's depth on every write path, not only in admission                      | Finding 11: not this plan's hole. Recommended as its own issue, for the same reason structure 1 bounded an outline at 64 | **A small content-model fix**                        |
