...
| 2   | [The layout](2026-09-19-publishing-02-the-layout.md)                           | Outline schema 2 with `front` matter, first and numbered in its own scheme (#152, STR-064); in `packages/domain`, the layout closed at its first version and the product's default; migration 0018 seeding it in every environment and recording it on each request and publication; a document refused at the door in another language than its layout (#144, PUB-095) or for a format it does not make; template `publication/2` with the page, a cover, running heads and feet, page numbering per matter in chains and a contents tagged as one. Lists of figures and caption labels move to slice 3, and so do the two tasks this slice did not build: the layout's scheme reaching the routes, and the panel | Built (PR #153) |
...
top level (#152, STR-064), with its own rule per sequence in the numbering scheme. A publish is
refused at the door for a format the layout does not make or a document in a language the layout's
words are not written in (#144, PUB-095), naming both tags.
...
the document would fix it. **Tasks 9 and 10 were not built**: the layout's scheme does not reach the
numbering route, the document view or the outline panel, and the panel does not offer front matter,
so front matter is reachable only through the API and STR-036 is still cited by nothing. Both move to
slice 3, and are named in publishing.md, structure.md, architecture.md and at the end of the plan. It
cites PUB-007, PUB-008, PUB-009, PUB-011, PUB-014, PUB-037, PUB-079, PUB-088, PUB-095 and STR-013;
1,384 requirements, 408 claims, 204 citations.
