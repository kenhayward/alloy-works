...
| 4a  | [Workers and object storage](2026-09-11-scaffolding-04a-workers-and-object-storage.md)  | The job queue in the platform schema, `apps/worker` claiming and running jobs, a store credential per tenant, and one job kind end to end: a sample PDF rendered by the pinned Typst   | Written         |
| 4b  | Images and the full stack                                                               | Images for the service and the worker, the whole compose stack, and the image build in CI                                                                                              | Not yet written |
