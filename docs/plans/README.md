...
| 2   | [The layout](2026-09-19-publishing-02-the-layout.md)                           | Outline schema 2 with `front` matter, first and numbered in its own scheme; in `packages/domain`, the layout closed at its first version and the product's default; migration 0018 seeding it in every environment and recording it on each request and publication; a document refused at the door in another language than its layout (#144, TPL-059) or for a format it does not make; template `publication/2` with the page, a cover, running heads and feet, page numbering per matter and a contents tagged as one; the panel numbering with the layout's scheme. Lists of figures and caption labels move to slice 3                                                   | Planned         |
