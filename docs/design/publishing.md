...
| **PUB-031** | Blocks are published in the document's order and tagged in it: a figure its style floats is tagged where it stands among the blocks, though Typst draws it at the head or the foot of the page, and in Word its text box is anchored in a paragraph of its own at the same place. Measured: a head float is read between the paragraphs either side of it, drawn above both; a foot float before the paragraph after it, drawn below it (W1.3)                                                                                                                                                                                                                                                                                                                                             |
...
| **PUB-069** | Each run carries its passage's language to the engine - a component's own, a language mark's - and Typst hyphenates by it and breaks lines by it; in Word it is the run's `w:lang`, which Word hyphenates by where the document asks it to. A language the pinned Typst has no hyphenation patterns for is not hyphenated at all, never as the document's. Measured on Typst 0.15.1: it has patterns for Afrikaans, Albanian, Catalan, Croatian, Czech, Danish, Dutch, English, Estonian, Finnish, French, German, Hungarian, Icelandic, Italian, Kurdish, Latin, Lithuanian, Norwegian (both), Polish, Portuguese, Slovak, Slovenian, Spanish, Swedish, Turkish and Turkmen; none for Basque, Galician, Irish, Latvian, Romanian or Welsh, among the Latin-script languages probed (W1.3) |
...
| **STR-027** | `assemble` prints `number`, `title` and `numberAndTitle` from resolution (structure.md's `referenceResolver`); the template prints `page`, from `counter(page).at` at the target's label, in its matter's own numbering; `relative` is the layout's `above` or `below` by document order, which layout schema 3 adds (structure.md, XR-C). Each is read back from the PDF                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **STR-029** | `assemble` resolves every reference in the document it publishes, and each whose target the document does not hold - or a `component` target whose component it places never or twice - fails `cross_reference_unresolved` at `compose`, naming the occurrence, the reference as its block and its stored target in `detail`, never the author's text; any fails the publish                                                                                                                                                                                                                                                                                                                                                                                                               |
...
a cross-reference its target's anchor, the text it displays or none where Typst must supply the
page, and whether it is a link. **An anchor names an occurrence and a block together** -
`b-<node>-<block>` - because one component placed twice holds the same block identifiers twice
...
section's title, which carries a reference as its number, already in its words. **Every block, footnote and node carries
`anchor`, and it is null unless a reference names it**, so the file holds a label for every
reference and no more; a named block that publishes nothing - an empty paragraph, a list whose items
are all empty - leaves a `{ type: 'marker', anchor }` in its place, set as `metadata(none)` with the
label, which a link and a page both find. The failures join the list: `cross_reference_unresolved`,
where resolution fails (STR-029, STR-062), naming the stored target in `detail`, and
`cross_reference_form_unavailable`, naming the form, where the target has none to print - a number
or a title asked of a paragraph - where a section's title asks for a page, where a relative form is
asked under a layout with no words for it, and where the target stands in a table's header rows,
which the engine sets again on every page with the label, and a label set twice refuses the compile.
Each names the occurrence the reference is read in, or the section whose title holds it, and the
reference as its block. They are resolved once, after the layout's own checks and before anything
is projected, so they come in document order after the layout's failures and before the content's
...
one node, the slice divides as figures 2 and 3 did, not by where the image stands. **Figures 5 built
the last**: an image in a run of text published one line high, in the room where it stands - a table's
...
schema 2 layout reads as having neither - a migration cannot know another language's words - and a
relative reference under one fails `cross_reference_form_unavailable`, naming the form. **The
default layout's version 0.4** carries _above_ and _below_, inserted by migration 0023 with the same
guard as 0021's: only where the environment's layout is still 0021's own unauthored 0.3. A request
...
'body' | 'code')` - because `raw` would otherwise fall back to Serif for a character Mono lacks and
   ...
   `line_too_wide`, because the engine neither clips nor reports one - it runs off the page. Building
   it found that the engine pads a block quotation by an em on **both** sides, so a quotation costs two
