...
| **STR-027** | `number`, `title` and `numberAndTitle` come from `references`; `page` from Typst, at the label; `relative` is "above" or "below" by document order, in the layout's words, which a layout schema version adds (structure.md, XR-C)                                                                                                                                                    |
...
| PUB-022                              | Cross-references are internal links in a paragraph's text, and text in a caption or a title, as measured (structure.md, XR-D). Citations cannot be until LIB gives them entries to link to                                                             |
