...
| Answered    | Whether inline JSONB holds up at authoring volumes. Measured by [the version chain plan](../plans/2026-09-15-storage-01-the-version-chain.md)'s load test at 200,000 components: every measure passed - a cut at p95 6.01 ms, opening a component at p95 3.41 ms, 900 versions' content in one read at p95 224.4 ms - and the chain projects to 30.7 GB for a tenant of a million components, so content stays inline. What it did not measure - a chain several times the machine's memory - stays open with the hosting decision |
