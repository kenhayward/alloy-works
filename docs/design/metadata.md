...
| New | Whether `09:00` and `09:00:00` are one value for `fixed` and default comparison and the digest. `checkValue` already treats them as one for a range or a duplicate, but `fixed` and default comparison and the digest compare canonical bytes, so the two spellings are two values there today; settle before the storage plan composes the digest                                |
