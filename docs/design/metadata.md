...
| **MET-038** | `checkUserValues` checks that every `user` value names a principal of this tenant, over a tenant-scoped lookup of its directory, and the service runs it on every iteration it accepts and refuses on a failure, `metadata.user` ([User values](#user-values))                                                                                            |
...
| MET-039                     | MET-029 was split. Its T1 half, MET-038 - a value a user of this tenant - is claimed above. MET-039, a value staying readable after its user leaves, is T2's: showing a departed user as no longer active is [component-editor.md](component-editor.md)'s for a component and undesigned for a document or a section                                                                                                                                                    |
...
| MET-037                     | Refusing a field version that would make a schema's default invalid is the definitions-management service's, which is not designed. Resolution refuses to resolve one meanwhile, below. It does not wait on where-used, for MET-040's reason                                                                                                                                                                                                                            |
| MET-040                     | Refusing a new schema version that conflicts "at any place" is the definitions-management service's, which is not designed. It does not wait on where-used (MET-025, T2): in T1 the places a schema applies are component types and template assignments, found by a query over the definitions, not by MET-025's counts. MET-035, which MET-040 supersedes, also refused a change that would make a relationship invalid, which REL-054, a Constraint, already refuses |
...
refuses disagreeing defaults at assignment, MET-040 refuses a schema version that would introduce them,
...
nothing acts on it, and MET-040 and MET-037 wait on the definitions-management service that acts.
Neither waits on where-used (MET-025, T2): in T1 the places a schema applies are component types and
template assignments, found by a query over the definitions, not by MET-025's counts. So resolution checks every effective
...
MET-040 is meant to refuse, and until that is designed it is found only by running `checkAssignment`
over the type again.
...
left, which MET-039 forbids. Of the three, accepting is the only rule that breaks neither.
