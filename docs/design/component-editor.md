...
| Left unclaimed                              | Why                                                                                                                                                                                                                                           |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CNT-165                                     | CNT-035 was split. Its T1 half, CNT-164 - the other seven marks CNT-031 lists - is claimed above. CNT-165, **the defined term**, is T6's: its control needs a term to choose, and terms are LIB's library                                     |
...
| Undo and redo             | An identifier no other block holds is kept, so a block put back is the block it was; one another block holds goes to the descent rule, as a split's does                                                                                                                                                                                                    | Put back with the text, carrying the identifiers it had                                                                                    |
| Paste, from anywhere      | The admission pipeline's re-identify stage allocates a new identifier for every block before ProseMirror sees the slice (CNT-132), and the editor keeps what it allocated. A reference left behind whose block is gone is then pointed at the identifier that block arrived under, so a cut and a paste re-points it and a copy and a paste changes nothing | The same stage allocates a new identifier for every mark, and drops annotations whose owner does not travel (CNT-133)                      |
...
**An undo keeps what it puts back** (structure.md's XR-E, built by cross-references 1). The identity
plugin keeps an identifier exactly one node holds in a transaction that is an undo or a redo, or that
says it has named what it places - a paste, through a `keepsIdentifiers` meta - and judges everything
else by the descent rule as before. So deleting a table and undoing brings back the table every
reference points at, and a footnote put back by an undo keeps its own identifiers, which footnotes 1
recorded as renamed. ADR-0023's rule still re-identifies a block holding an identifier another block
keeps, so this is a narrowing of when the rule is asked, not a new rule. A cross-reference's own
identifier is allocated as a block's is.
...
**Footnotes, designed on 2026-09-23** with publishing.md's [Footnotes](publishing.md#footnotes), whose
decisions FN-A to FN-F these follow:

| Footnote anchored to | Made by                                                                                   | Survives                                                                                                  |
| -------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| A span of text       | **Footnote** on the toolbar, or `Ctrl+Alt+F` as Word has it, at the cursor in a paragraph | Any edit that keeps the mark; in a table's cell it moves with its row, since it stands in the cell's text |
| A cell, by key       | Nothing in T1 (FN-A): a table whose rows are generated is T2's (CNT-039)                  | Opened and saved as it was stored; publishing resolves it or refuses it (CNT-042)                         |
| A cell, by position  | Nothing in T1, as by key                                                                  | As by key                                                                                                 |
| The table as a whole | Nothing (FN-C): a note on a table is the table's **note**                                 | Refused at publish, by name                                                                               |

- **A footnote stands in a paragraph and nowhere else** (FN-B) - running text, a list's item, a
  quotation, a table's cell. **Footnote** is unavailable anywhere else, and a stored footnote in a
  caption, a term, an attribution or a table's note opens the component read-only, naming it, as an
  inline image there does.
- **Its mark** is an inline atom in the text, drawn as a small raised marker that a screen reader
  announces as _Footnote_. It shows **no number**: the number is the outline's (CNT-041), and a
  component opened on its own has none. Deleting the mark deletes the footnote, and `Ctrl+Z` brings it
  back whole.
- **Its paragraphs are edited inline** (FN-E), beneath the paragraph the mark stands in, in a nested
...
- **The table's note** (CNT-038) is added and removed from the Table panel (**Add note**, **Remove
  note**) and edited in place beneath the table, a line of inline content as a caption is.
...
- **Reference** on the toolbar, or `Ctrl+Alt+X` (`Cmd+Option+X` on a Mac), opens a dialog listing what
  may be pointed at - in a document's page, its sections and every figure, table, footnote and
  numbered block equation its components hold; in a component on its own, the component's own - each
  named as the reader will see
  it, then the forms the chosen target has and a line saying what it will show. **Insert** places the
  reference at the end of the selection and selects it whole; with a reference selected whole the
  dialog opens on it and **Change** changes its target or form, keeping its identifier. It is worked
  by keyboard as the other dialogs are, and nothing is placed on Cancel or `Escape`. **Reference** is
  unavailable in preformatted text and with a table selected whole, and available in a footnote's open
  editor.
- **The node** is `crossReference`, an inline atom carrying the stored identifier, target, display
  and `withoutPages`, with no marks and no `parseDOM`. It stands wherever inline content does - a
  paragraph, and so a list's item, a quotation and a table's cell; a footnote's paragraph, a term, an
  attribution, a table's or a figure's caption and a table's note - and never in preformatted text. A
  mark put over words and a reference rests on the words alone, and an annotation either side of one
...
- **A numbered block equation can be pointed at** (equations 2): the **Reference** dialog offers it
  by its number, _Equation 1_, with the forms number, page and above or below - no title, since what
  it says is maths - in a document and in a component on its own. One left unnumbered is not offered,
  having no number to print, and a reference to an equation the page has not numbered yet shows
  _Equation_.
...
- **It carries no marks**, since the stored node carries none: a mark put over words and an image rests
  ...
  one - changed or taken off whole, and carried on by text typed straight after the image - because the
  stored model ends an annotation only at text without it. The final review found the first draft saying "two pieces" and doing worse: the image took the
...
| A cross-reference, citation, variable or binding | An atom naming its target where the target can be read, and a marker where it cannot. A cross-reference shows what it will print in the document it is edited in, and on its own the target's kind and caption, since its number is the document's (structure.md, XR-C); one whose target has gone is shown as broken (XR-F) |
