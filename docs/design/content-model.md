...
| CNT-166                   | Text is JSON strings, so every character is storable and read back, the astral planes included; the editor shows and saves them, and a publish refuses one no face can set as `glyph_missing`, naming its code point, for the PDF and Word alike - each tested. But two things fall short of "the full Unicode range": U+0000 is refused, since Postgres cannot store it in `jsonb`, and text is kept in NFC (CNT-056), so the code points NFC replaces - among them the 555 CJK compatibility ideographs in the supplementary plane - are stored as their canonical equivalents. Unclaimed until CNT-166 says so |
