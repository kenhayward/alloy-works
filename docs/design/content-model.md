...
Seventy-seven requirements above, and the ones deliberately left out matter as much.
...
| CNT-166                   | Text is JSON strings, so every character but U+0000 is storable and read back, the astral planes included; the editor shows and saves them, and a publish refuses one no face can set as `glyph_missing`, naming its code point, for the PDF and Word alike - each tested. Two things fall short of "the full Unicode range": U+0000 is refused, since Postgres cannot store it in `jsonb`, and text is kept in NFC (CNT-056), so the 1,120 code points NFC replaces - 555 of them in the supplementary planes, 542 of those CJK compatibility ideographs - are stored as their canonical equivalents. Unclaimed until CNT-166 says so |
| CNT-061, CNT-062          | A Markdown and an HTML reader feed the same pipeline and keep every structure a component can hold, the report naming what they changed. A heading is kept as a paragraph, since a component holds none, and "must preserve structure" makes no exception for it; CNT-167 was reworded to say so for Word, and until these two are they are unclaimed                                                                                                                                                                                                                                                                                  |
