...
| **CNT-166** | Text is JSON strings, so every code point is storable, the astral planes included, and a version gives back exactly what was written. The editor holds and shows it through the same mapping, and saves what is typed among it whole. A publish judges each character against the theme's faces a whole code point at a time and refuses one no face can set as `glyph_missing`, naming it as `U+XXXX`, for the PDF and for Word alike, since `assemble` serves both (STY-049's check) |
...
Seventy-eight requirements above, and the ones deliberately left out matter as much.
