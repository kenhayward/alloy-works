...
> component version resolved for each occurrence on `GET /v1/documents/{id}`, and a title edited with
> a mark or a cross-reference in it. **A title is edited as inline content** since
> [equations 3](../plans/2026-09-24-equations-03-equations-in-a-sections-title.md), for words and
> inline equations; a title holding anything else is shown read-only. A cross-reference's stored shape - its identifier, its
...
| **STR-067** | The outline is the document version's content, so every change to it is a new version of the document; `baseline_pin` pins a document version like any other (VER-018)                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **CNT-160** | A reference's `mode` - the latest version, or one pinned - is a switch an outline `set` changes, and every act on an outline is recorded as a new version of the document in the version chain, naming its author and when it was made, with the whole outline as it then stood: so which version a document referenced before a change and after it are both read back from the chain. The audited act is CNT-161's (T3)                                                                                                                                                                                 |
...
| **STR-068** | A reference's target is a closed union - `node` for an outline node, `block` and `component` for a caption-bearing block or a footnote - each naming its target by identifier, never by number or position; a component may hold a `node` target (XR-B), so body text can point at a section                                                                                                                                                                                                                                                                                                              |
| **STR-065** | The contents panel is the outline, rendered: it renders the outline every operation returns rather than a copy of one, so it updates with every act the author makes, and clicking a node takes the reader to it ([Navigation](#navigation))                                                                                                                                                                                                                                                                                                                                                              |
...
Forty-five claims above. The requirements deliberately left out are where this design's edges are, and
...
| STR-066                   | STR-034 was split. Its T1 half, STR-065 - the contents updating as the author changes the outline - is claimed above. STR-066, **another person's change**, is T3's: a colleague's change reaches the panel only when the author next acts and is refused, or reloads. The stream carries no document version, and one would have to be withheld from every viewer who may not read the document - a design of its own                                                                         |
...
| CNT-046                   | Claimed by [component-editor.md](component-editor.md), which makes an equation in every context it names, a heading among them in the outline's title field (equations 3). This design's share is the title being inline content, which it is                                                                                                                                                                                                                                                  |
| CNT-079, CNT-072          | Exposing structure to assistive technology, and one continuous scroll, are the document view's                                                                                                                                                                                                                                                                                                                                                                                                 |
| TPL-027, TPL-015, TPL-013 | A document owning its outline after instantiation, what a template lets an author change, and a missing required section (TPL-013; TPL-030, which repeated it, is withdrawn) are all about instantiation, which is **TPL**'s                                                                                                                                                                                                                                                                   |
...
**Title** and **Starts on** - reached with `Tab` like any other. **Title** is a one-line editor of
words and inline equations, with **Equation** beside it (equations 3): `Enter` commits the title,
leaving it commits too, and its own undo is `Ctrl+Z` while the focus is in it. The tree itself is one tab stop with
...
| `{ kind: 'node', node }`                  | An outline node - a section, or a component reference's own heading                                    | A section title, and a component since XR-B |
...
CNT-125's "any block can be a target", which content-model.md claims for the model's half, is
demonstrated for an equation too: the resolver's test binds a numbered equation by its number and an
unnumbered one by its page and place alone, and cites it.
...
### Making, showing and printing a reference

Designed on 2026-09-23, after footnotes, and measured against the pinned engine as footnotes were.
Structure 4 designed how a reference **resolves**, above; nothing yet says how an author makes one,
what it shows while they write, or what a publication prints. The component editor's matrix says a
reference is made "in the document view", which nothing plans. This section answers those, with
decisions XR-A to XR-G for Ken.

#### What the pinned Typst does with a reference, measured

Throwaway files compiled by the pinned Typst 0.15.1 with the worker's arguments - PDF/UA-1 and
`--features a11y-extras` - checked by the pinned veraPDF and read back from the file's structure tree.

| Case                                                                                              | Result                                                                                                                                            |
| ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `link(label(..))[..]` to a table's figure, a paragraph, a heading and a footnote                  | A `Link` holding the text and its annotation, inside the `P` it stands in. veraPDF passes, 0 rules failed                                         |
| A page: `context`, `locate(label)`, `counter(page).at(..)` in the location's own `page-numbering` | The target's page number in the numbering of the matter it stands in, linked or not. veraPDF passes                                               |
| In a footnote's text                                                                              | A `Link` inside the `Note`. veraPDF passes                                                                                                        |
| **In a table's caption**                                                                          | The list of tables sets the caption again, so its entry - itself a `Link` - holds a second `Link`: two targets under one click. veraPDF passes it |
| **In a heading**                                                                                  | The same, in the contents                                                                                                                         |
...
**Reference** on the toolbar, with a shortcut the plan measures free, opens a dialog - never typed
inline, because a target is chosen, not spelt. It lists what may be pointed at, each shown as the
reader will see it, and is worked by keyboard as the other dialogs are (CNT-077):

- **In a document**, where a component is edited in place on its document's page, the dialog offers
  that document's sections by number and title, and every figure, table and footnote the document's
  components contribute, by label and caption - everything the page already reads from the outline,
  the numbering and the contributions routes. This is the "document view" the matrix waited for: the
  page that places a component is where its references mean something.
- **A component opened on its own** offers its own figures, tables and footnotes, by kind and caption,
  since it has no numbers without a document. It offers no section and no other component's block.

The dialog then offers the forms that target has (XR-C) and shows what the chosen one prints.

#### Which target is stored (XR-B)

A block of the component being edited is stored as a `block` target, and another component's as a
`component` target, as designed above. **A section is stored as a `node` target, and a component may
now hold one**: body text saying "see Section 4.2" is the commonest reference there is, and the gap
STR-026 was left unclaimed for, which STR-068 now asks to be closed, is exactly that. A node belongs to one document's outline, so such a reference resolves in
the document that holds the node and **fails by name in any other** the component is placed in
(STR-029) - loud, never wrong. It is a widening of the walk's rule, `checkInlineContent`, and changes
nothing stored. A bibliography entry still waits for LIB.

#### What a reference shows (XR-C)

| Form             | Prints                                                                                                   | Offered for                              |
| ---------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| `number`         | The numbering table's label: "Table 1.1", "2.1" for a section, "3" for a footnote                        | A section, a figure, a table, a footnote |
| `title`          | A section's title; a figure's or a table's caption words                                                 | A section, a figure, a table             |
| `numberAndTitle` | Both, as a list after the contents sets them                                                             | A section, a figure, a table             |
| `page`           | The target's page number, in its matter's numbering                                                      | Anything, a paragraph included           |
...
caption - _Table: Readings_ - since its number is the document's. It carries no marks, as a footnote's
mark carries none, and deleting it deletes the reference.
...
A reference is only as good as its target's identifier, and two things the editor does today break it:

- **An undo gives a block it puts back a new identifier** - measured in footnotes 1, where the descent
  rule names whatever is placed. So deleting a table and undoing leaves every reference to it pointing
  nowhere. **An undo keeps an identifier no other block holds.**
- **A paste gives every block a new identifier** (CNT-132), so moving a figure by cut and paste orphans
  its references. CNT-132 stands: the pasted block is still newly named. **After a cut and a paste in
  the same component, the editor re-points the references left behind at the block that came back.**

#### Failures (XR-F)

In the editor, a reference whose target has gone is shown as broken, naming what it pointed at. At
...
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| XR-A | **References are made from a dialog in the component editor**: in a document, over its sections and its components' figures, tables and footnotes; on its own, over the component's own. No separate document view is built | Yes. The document page already edits a component in place and already holds the outline, the numbers and the contributions. A document view is its own design, and references need none of it |
| XR-B | **A component may hold a `node` target**, so body text can say "see Section 4.2", failing by name in a document without that node                                                                                           | Yes. The alternative - an arm meaning "the section that places this component" - survives reuse but cannot reach a section that places nothing, and most references are to such sections      |
| XR-C | **All five forms**, `relative` by document order in the layout's words, with a layout schema version adding "above" and "below"                                                                                             | Yes, keeping publishing.md's STR-027 claim whole. Or cut `relative` from T1 - authors type "above" themselves - and drop the claim, naming the gap                                            |
| XR-D | **A link in a paragraph's text, text elsewhere**                                                                                                                                                                            | Yes: measured, a link in a running head refuses the compile and one in a caption or a title nests in the lists and contents                                                                   |
| XR-E | **An undo keeps identifiers; a cut and paste re-points the references left behind**                                                                                                                                         | Yes. Without both, the two commonest edits to a document with references silently break them, and the break is found only at publish                                                          |
...
of one; so STR-065 is answered, and another person's change, STR-066, is T3's, as "What this
document does not own" says. Reordering from the panel is the move operation (STR-037). **The panel goes to a linked node,
...
| `packages/editor`       | The title editor: one ProseMirror view per node title, over an inline-only schema - `title.ts` and `titleView.ts`, words and inline equations, no marks (equations 3); a title holding a mark, a cross-reference or a footnote is shown read-only until the schema takes them                                                                                                                                                                                                                                                                                                             |
