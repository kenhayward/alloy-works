...
| STR-026                   | The target union is built without one of the targets it needs, so it is not claimed. **A bibliography entry**: no `entry` arm until **LIB** says what an entry's identity is. **A component's reference to a section** was the other gap: a `node` target stood in a section title alone, so body text could not say "see Section 4.2". [Making, showing and printing a reference](#making-showing-and-printing-a-reference) proposes letting a component hold a `node` target (XR-B), which closes it once built; the bibliography entry remains |
