...
This repository holds 1,362 product requirements. Every one of them has an identifier, and the point
...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| What does this product claim to do?                          | [`../specification/requirements/`](../specification/requirements/) - 1,362 requirements, 22 areas |
...
**`0.13.0` declares seven requirements and excludes four, each with a stated reason.** Seven out of 1,362 is not an error. This product is scaffolding: it can sign in, isolate a tenant, run a job and
...
| `pnpm trace area <XXX> \| --all --file <filename>` | The same listing written to a file as UTF-8, with a byte-order mark and the line endings of the platform that wrote it - CRLF on Windows, LF elsewhere - so it reads correctly in that platform's own text viewers. A relative filename is taken from the repository root, wherever inside the repository the command is run, and a folder that does not exist is refused rather than created |
