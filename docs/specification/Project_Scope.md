...
- **Organisation** - a customer. It holds one or more tenants - production, and a sandbox or a
  validation environment as the customer needs - and shares billing, administration and identity
  provider configuration across them, never content. See
  [ADR-0020](../decisions/0020-service-foundations-tenant-roles-zod-first-apis-kysely.md).
- **Tenant** - one environment of a customer. The isolation boundary for identity, content,
  connections, model endpoints, secrets and audit. Nothing crosses a tenant boundary.
...
| 22  | **A request reaches one tenant by hostname, session and role**                               | A customer's environments are separate tenants under one organisation, each at its own hostname. Sessions live in the tenant that issued them; every database access assumes the tenant's role for one transaction, and the login roles can touch no tenant table themselves. Endpoints are zod schemas on Fastify with the OpenAPI document generated and committed; queries are Kysely; migrations are SQL applied to every schema by our own runner. See [ADR-0020](../decisions/0020-service-foundations-tenant-roles-zod-first-apis-kysely.md) |
