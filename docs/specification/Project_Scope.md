...
| 11  | **Tenants are isolated by schema**                                                           | One database, a schema per tenant, enforced by role rather than by application code. It keeps the stronger option open: moving a schema into its own database later is mechanical, while splitting pooled rows is a migration of everything. See [ADR-0008](../decisions/0008-schema-per-tenant-isolation.md)                                                                                                                   |
| 12  | **Authentication is federated, and the product never holds a password**                      | A tenant's own provider over OIDC, or Google accounts where no federation exists yet. Only basic identity scopes, so no app verification. See [ADR-0009](../decisions/0009-federation-and-google-accounts-no-local-passwords.md)                                                                                                                                                                                                |
| 13  | **Product-supplied typefaces are open-licence only**                                         | A customer's brand face is supplied by the customer under their own licence. Same reasoning as decision 10: no recurring, deployment-scaled licence in the path between a document and a reader. See [ADR-0010](../decisions/0010-open-licence-typefaces-only.md)                                                                                                                                                               |
...
no step log. The pagination and PDF engine is the remaining irreversible one.

| #   | Decision                      | What it hinges on                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Pagination and PDF engine** | Constrained by [ADR-0007](../decisions/0007-no-per-server-licensing-in-the-publishing-pipeline.md) to open-source candidates. **The four gates have run: Typst passes all four, and nothing else passes more than two** - see [`Publishing_Engine_Spike_Findings.md`](Publishing_Engine_Spike_Findings.md). What remains is cases 5 to 9, and whether ADR-0005's XHTML intermediate is narrowed to let the resolved content model feed Typst directly |
| 2   | **Word generation approach**  | Direct OOXML or an intermediate representation. Constrained by ADR-0005's commitment to design the schema against the OOXML mapping, and by the engine decision above                                                                                                                                                                                                                                                                                 |
| 3   | **Search infrastructure**     | Whether full-text and semantic search are one system or two, and how the permission filter is applied at query time without leaking existence                                                                                                                                                                                                                                                                                                         |
| 4   | **Relationship storage**      | Recursive SQL or a graph store. Hinges on realistic traversal depth and volume                                                                                                                                                                                                                                                                                                                                                                        |
| 5   | **Realtime transport**        | Presence, locks, notifications and streaming, and whether one channel serves all four                                                                                                                                                                                                                                                                                                                                                                 |
