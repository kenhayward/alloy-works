...
> **Status: v1, reviewed.** The first detailed requirements area. It rests on a
...
| [MET](MET-metadata-and-component-types.md)                                    | A component's type, and the fields its metadata values are validated against             |
...
| A component's type, and the fields and schemas its values are validated against | **MET**          |
...
**A component's metadata is [MET](MET-metadata-and-component-types.md)'s.** A component's own title and the attributes every component carries are CNT-142, CNT-143 and CNT-145; its component type, and the fields its values are validated against, are **MET**'s, which settled **CNT-Q12**. **SCH** searches the values.
...
| **CNT-005** | Accepting, rejecting or excluding a marked annotation must act on every fragment of that identifier in one operation                                                                                                                                                                                                             | T3         | Specified             |
...
| **CNT-009** | A resolution pass (accepting a suggestion, excluding a condition) must preserve block identifiers                                                                                                                                                                                                                                | T3         | Specified             |
...
| **CNT-142** | A component must carry a title of its own, distinct from the title of any outline section that places it (**STR-060**), so that it can be listed and found in a space holding no documents at all                                                                                                                                | T1         | Specified             |
| **CNT-143** | A component's title and base language (CNT-140) must be carried in its versioned content, so that a version records the title the component had when it was cut                                                                                                                                                                  | T1         | Specified             |
| **CNT-144** | A component's structural attributes - those every component carries whatever its type - must be a closed set: its identifier, its title, its base language, and the schema version its content was written against. Adding one is a schema change with a migration, not a configuration option                                   | Constraint | Superseded by CNT-145 |
| **CNT-145** | A component's structural attributes - those every component carries whatever its type - must be a closed set: its identifier, its component type (**MET-011**), its title, its base language, and the schema version its content was written against. Adding one is a schema change with a migration, not a configuration option | Constraint | Specified             |
| **CNT-146** | A component's versioned content must hold a closed set of members at its root - the schema version, the title, the base language, the base direction and the blocks - and a metadata value must never be one of them (**MET-016**)                                                                                               | Constraint | Specified             |
...
**CNT-142 closes a gap the specification carried from the beginning.**
[`Project_Scope.md`](../Project_Scope.md) §6 defines a component as "a small, typed, titled,
independently revisable piece of content" that "knows its own content and metadata", and no area
picked any of that up. The only title in the corpus was STR-005 - an outline section title, which
belongs to the position a component occupies rather than to the component. Left that way a
component's title becomes whatever heading places it, which is wrong the first time the component
is transcluded under a different heading, and wrong immediately for search: SCH-002 covers titles,
and a space holding no documents has no heading to cover.

**CNT-143 puts the title where a version can record it.** A published document has to be able to
say what its components were called at the moment it was published, so the title travels with the
versioned content rather than beside it, on the same argument as CNT-011 - content that leaves the
database carries what it needs to describe itself.

**CNT-145 and CNT-146 separate two closures that CNT-144 held as one.** CNT-145 is what every component carries, and it now includes the component's type (**MET-011**), which is recorded on the version rather than written into content. CNT-146 is what the content document's root holds, which is narrower, and it is the closure a content hash rests on. Metadata values are in neither: they belong to the version, beside its content (**MET-016**). CNT-144 is superseded rather than edited because adding the type changes what the closed set holds. The set being closed is still what stops a field arriving in the schema by accident, which is the failure **CNT-Q12** named.
...
| **CNT-122** | A named image style must be resolved to real dimensions at publish time, by the rules **STY** owns (STY-015 to STY-019), and the editor must resolve it by those same rules (STY-035)                                                                                                      | T1         | Specified             |
| **CNT-123** | An image reference must carry no pixel, point, millimetre or percentage dimension of its own                                                                                                                                                                                               | Constraint | Specified             |
| **CNT-091** | Resolving an image style must preserve the image's intrinsic aspect ratio. A style that fixes a width must derive the height from the image, and must never distort it                                                                                                                     | T1         | Superseded by STY-016 |
| **CNT-092** | A style whose resolved height would exceed a declared maximum must be constrained by height instead, still preserving ratio. That maximum must be declared in the style, not discovered at publish time                                                                                    | T1         | Superseded by STY-017 |
| **CNT-093** | Style catalogues must be configuration-extensible: an administrator must be able to add a style - "small thumbnail" - without a code change, and it must then be selectable by authors and honoured by every output format                                                                 | T2         | Superseded by STY-029 |
...
**CNT-Q15 is settled, and the duplication is gone.** CNT-091 and CNT-092 said what STY-016 and
STY-017 say, and CNT-093 said what STY-029 and STY-030 say - the condition under which two documents
drift apart while each looks correct. The line that survives: content carries a named style and no
dimension of its own; the style resolves, in **STY**.
...
| **CNT-035** | The author must be able to apply and remove every mark in CNT-031 directly, from a toolbar and by keyboard shortcut. The editor must offer no control over typeface, font size or colour                                                                                                                             | T1      | Superseded by CNT-164 |
| **CNT-164** | The author must be able to apply and remove emphasis, strong, underline, subscript, superscript, inline code and a quoted phrase directly, from a toolbar and by keyboard shortcut. The editor must offer no control over typeface, font size or colour                                                              | T1      | Specified             |
| **CNT-165** | The author must be able to apply and remove a defined term directly, from a toolbar and by keyboard shortcut (**LIB-013**)                                                                                                                                                                                           | T6      | Specified             |
...
| **CNT-152** | Where an author marks a run of text with a language the product's outputs cannot carry, the editor must name the tag at the time, before the mark is applied, rather than leaving it to be discovered at a refused publication                                                                                       | T1      | Specified             |
| **CNT-085** | An underline mark must be supported                                                                                                                                                                                                                                                                                  | T1      | Specified             |
...
| **CNT-051** | Bibliography entries must be managed artifacts within a space, referenceable from any component in it                  | T6      | Specified |
...
| **CNT-054** | A citation whose entry cannot be resolved must fail the publish, naming the citation and the component holding it      | T6      | Specified |
...
| ID          | Requirement                                                                                                                                                                                                   | Tranche | Status                |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | --------------------- |
| **CNT-055** | The full Unicode range must be storable and renderable, including characters outside the Basic Multilingual Plane                                                                                             | T1      | Superseded by CNT-166 |
| **CNT-166** | The full Unicode range must be storable and shown in the editor, including characters outside the Basic Multilingual Plane; a character a publication's typefaces cannot set must fail the publish, naming it | T1      | Specified             |
...
| ID          | Requirement                                                                                                                                                                           | Tranche | Status                |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | --------------------- |
| **CNT-060** | Paste from Word must preserve structure - headings, lists, tables, footnotes, emphasis - rather than producing styled runs                                                            | T1      | Superseded by CNT-167 |
| **CNT-167** | Paste from Word must preserve structure - lists, tables, footnotes, emphasis - rather than producing styled runs; a heading must become a paragraph, named in the paste report        | T1      | Specified             |
| **CNT-168** | Paste from Word must keep a heading as a heading, breaking the paste into sections and components (Word import and breakout)                                                          | T6      | Specified             |
...
| **CNT-103** | Undo must not cross a version boundary. Reaching further back than the version the session opened from is restoring an earlier version, which is a separate and audited act                                                                                   | T1         | Superseded by CNT-169 |
| **CNT-169** | Undo must not reach back past the version the session opened from, nor past a version cut during the session                                                                                                                                                  | T1         | Specified             |
| **CNT-170** | Reaching further back than the version the session opened from must be restoring an earlier version, a separate and audited act (**VER-033**)                                                                                                                 | T3         | Specified             |
...
| ID          | Requirement                                                                                                                                                                            | Tranche | Status                |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | --------------------- |
| **CNT-104** | The document view must operate in one of three modes - **read**, **review** or **author** - and must make the current mode obvious                                                     | T1      | Superseded by CNT-154 |
| **CNT-154** | The document view must operate in one of two modes - **read** or **author** - and must make the current mode obvious                                                                   | T1      | Specified             |
| **CNT-155** | The document view must offer a **review** mode beside read and author (CNT-154)                                                                                                        | T3      | Specified             |
...
| **CNT-106** | Each mode must offer only what it is for: read offers navigation and no editing affordances; review offers comments and suggestions but not direct edits; author offers direct editing | T1      | Superseded by CNT-156 |
| **CNT-156** | Each mode must offer only what it is for: read offers navigation and no editing affordances; author offers direct editing                                                              | T1      | Specified             |
| **CNT-157** | Review mode must offer comments and suggestions but not direct edits                                                                                                                   | T3      | Specified             |
...
**CNT-108, CNT-109 and CNT-141 are the audited step back and step forward.** A document references a
...
| **CNT-N07** | **No tables or images inside footnotes** (CNT-129, which superseded CNT-040)                                                                                                                                                                             |
...
| **CNT-Q12** | **Who owns a component's own metadata - its title, its tags?** **TPL** owns the metadata schema a document declares and **SCH** searches the values, but no area states where a component's own title and values live, or what validates them                                                                                                                                             | **Settled** by [MET](MET-metadata-and-component-types.md). A component is of exactly one component type (MET-011), which assigns the schemas its fields come from, and a component never takes fields from a document that references it (MET-013), so transclusion into documents from different templates changes nothing. CNT-145 adds the type to the attributes every component carries |
| **CNT-Q13** | **What is the reference configuration the preview budget is measured on (CNT-151)?** Two seconds on what, rendering what content                                                                                                                                                                                                                                                          | The first performance harness, which has to name a machine before it can report a number. A budget with no baseline can be neither confirmed nor refuted                                                                                                                                                                                                                                     |
...
| **CNT-Q15** | **Should CNT-091 to CNT-093 be superseded by their STY equivalents?** They said what STY-016, STY-017, STY-029 and STY-030 say, and two owners of one rule is how the two drift apart                                                                                                                                                                                                     | **Settled: yes.** CNT-091, CNT-092 and CNT-093 are superseded by STY-016, STY-017 and STY-029. This area keeps what content carries - a named style and no absolute dimension (CNT-121, CNT-123) - and **STY** owns how a style resolves. `docs/design/themes.md` claims the STY requirements directly                                                                                       |
...
| CNT-142 to CNT-144          | Scope §6, "Component - the atom ... a small, typed, titled ... it knows its own content and metadata"               |
| CNT-145, CNT-146            | [MET](MET-metadata-and-component-types.md); section 17, "From specifying metadata and component types"              |
...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1, 3.1.4, 3.2.6 | **CNT-110 superseded by CNT-141**, which shows which of three reference modes a reference takes rather than two. CNT-N07 now cites CNT-129 rather than superseded CNT-040. **CNT-Q15 is settled**: CNT-091, CNT-092 and CNT-093 are superseded by STY-016, STY-017 and STY-029, leaving this area what content carries - a named style and no absolute dimension - and STY how a style resolves |

### From designing the content model

Not a review. Designing the content model read this
document against [`Project_Scope.md`](../Project_Scope.md) §6 and found the specification asserting
something no area had written down.

| What was found                                                                                                                                                                                                                                                                                                              | Change                                                                                                                                                                                  |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Scope §6 calls a component "a small, typed, titled, independently revisable piece of content" that "knows its own content and metadata". No requirement in any of the twenty-one areas gave a component a title. The only title in the corpus was STR-005's, which belongs to an outline section rather than to a component | **CNT-142, CNT-143 and CNT-144 added**, giving a component its own title, carrying it in versioned content, and closing the set of attributes every component carries whatever its type |
| **CNT-Q12 is half settled.** The structural half is answered by the three rows above. The typed half - a component type declaring the metadata its components carry - is a subsystem rather than a row, and a component cannot take its type from a document because it is transcluded into many                            | CNT-Q12 rewritten to record the split and to name what would settle the remainder. Filed separately so it is specified rather than designed under pressure                              |
| Not changed: **CNT-002's component-scoped block identifier.** It looked inconsistent with STR-053's tenant-scoped outline identifier, and it is not - STR-056 requires a component referenced twice to resolve against the occurrence the reader is in, so a block identifier is deliberately not unique within a document  | No change here. What is missing is in **STR**: STR-026 says a cross-reference targets a block "by identity" without saying that the identity is the occurrence and the block together   |

| Counts           | Before                     | After                                     |
| ---------------- | -------------------------- | ----------------------------------------- |
| Requirements     | 141, of which 6 superseded | 144, of which 6 superseded                |
| Non-requirements | 10                         | 10                                        |
| Open questions   | 15, of which 8 settled     | 15, of which 8 settled and 1 half settled |

### From specifying metadata and component types

Not a review. Specifying [MET](MET-metadata-and-component-types.md) answered the typed half of
CNT-Q12, and in doing so changed a closed set this document declares.

| What was found                                                                                                                                                                                                    | Change                                                                                                                                                                    |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CNT-144 closed the attributes every component carries "whatever its type" and left the type itself out, because what a type was had not been specified. Every component now has exactly one (MET-011)             | **CNT-144 superseded by CNT-145**, which adds the component type to the closed set                                                                                        |
| CNT-144 also stood for a second closure it never stated: the members of the content document's root, which the content model built and which a content hash rests on. Metadata values must not enter it (MET-016) | **CNT-146** states the root closure on its own, so the attributes a component carries and the members its content holds can each be claimed by the design that answers it |
| CNT-Q12 was half settled                                                                                                                                                                                          | **Settled** by MET                                                                                                                                                        |
| Boundary and pointers                                                                                                                                                                                             | The boundary row for a component's metadata now names **MET** rather than TPL; CNT-142 cites STR-060, which superseded STR-005                                            |
...
Not a review. [The publishing design](../../design/publishing.md) challenged two of this area's preview
requirements, and Ken accepted both. Preview itself is still undesigned in full: the design shapes it
and leaves both rows unclaimed until its own slice.
...
| As filed, the statement ended "rather than letting the component be written and the publication be refused later", which reads as forbidding the component to be written at all - and the editor's answer is a warning the author may press past, so the design's claim would have looked like one exceeding its requirement           | A clarity edit, keeping **CNT-152**: it now says the editor must name the tag at the time, before the mark is applied, rather than leaving it to be discovered at a refused publication. What the product must do is unchanged - the tag is named before anything is written - so this is an edit and not a supersession                                     |
| CNT-152 asks only about a **run** an author marks. A component's own base language goes through the same rule and is already refused at a publish today, and the field that sets it checks the wide BCP 47 shape alone, so that half of the same harm is unwarned                                                                      | Not changed here, and named rather than left silent: [#156](https://github.com/kenhayward/alloy-works/issues/156) carries the base-language field, and [component-editor.md](../../design/component-editor.md) says beside the CNT-152 claim what is still unwarned                                                                                          |
...
| CNT-085 read "An underline mark must be supported, and must be understood as the one mark named for its appearance rather than its meaning". The second clause is a claim about how a document is read, which no test can demonstrate, so the requirement could never be more than design-answered (challenge 1 of the marks plan)     | A clarity edit, keeping **CNT-085**: the statement is now its first clause alone. The second is unchanged in substance and lives in [content-model.md](../../design/content-model.md), which says plainly that `underline` is the one mark named for its appearance and which claims CNT-085 on that term. Ken ruled this an edit rather than a supersession |
...
| What was found                                                                                                                                                                                    | Change                                                                                                                                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Accepting, rejecting and excluding act on suggestion marks, whose review is COL's in T3, and exclusion needs conditions, which are REU's in T4. Only the spike's draft model built it             | **CNT-005 moved to T3**                                                                                                                                                                                                                                     |
| Both passes CNT-009 names, accepting a suggestion and excluding a condition, arrive in T3 and T4                                                                                                  | **CNT-009 moved to T3**                                                                                                                                                                                                                                     |
| A tracked-changes view needs the comparison CNT-111 and CNT-112 make, which is T3                                                                                                                 | **CNT-113 moved to T3**                                                                                                                                                                                                                                     |
| Suggestions and comments, which CNT-137 announces, are T3                                                                                                                                         | **CNT-137 moved to T3**                                                                                                                                                                                                                                     |
| Suggestions, comment anchors and redlines, which CNT-138 distinguishes, all arrive in T3                                                                                                          | **CNT-138 moved to T3**                                                                                                                                                                                                                                     |
| Bibliography entries need citation styles and reference sources (**LIB**, CNT-053, STY-020), all T6                                                                                               | **CNT-051 moved to T6**                                                                                                                                                                                                                                     |
| An entry that cannot be resolved means nothing until entries exist (CNT-051, T6); today every citation already fails the publish by name                                                          | **CNT-054 moved to T6**                                                                                                                                                                                                                                     |
| Review mode needs suggestions and comments, which are T3                                                                                                                                          | **CNT-104 superseded by CNT-154** - read and author, T1 - with **CNT-155**, review mode beside them, in T3                                                                                                                                                  |
| CNT-106 said what review mode offers, and review mode is T3                                                                                                                                       | **CNT-106 superseded by CNT-156** - what read and author offer, T1 - with **CNT-157**, what review mode offers, in T3                                                                                                                                       |
| A revision is designated by LIF's gate, which is T3                                                                                                                                               | **CNT-108 superseded by CNT-158** - choosing a version, T1 - with **CNT-159**, choosing a revision, in T3                                                                                                                                                   |
| An audited act needs the audit log, which is T3; what T1 can hold is the change itself, as a version of the document                                                                              | **CNT-109 superseded by CNT-160** - the change recorded as a new version of the document, naming who and when, T1 - with **CNT-161**, the audited act, in T3                                                                                                |
| Tracking the latest approved revision needs LIF-038's latest approved revision, which is T3 (REU-050, which also names it, is T4)                                                                 | **CNT-141 superseded by CNT-162** - the version referenced, pinned or floating, T1 - with **CNT-163**, a reference tracking the latest approved revision, in T3                                                                                             |
| A defined term needs terms to choose from, and terms are LIB's, in T6. CNT-031 lists no link and no language, so CNT-164 names its marks rather than "every mark in CNT-031 but the defined term" | **CNT-035 superseded by CNT-164** - every other mark CNT-031 lists, T1 - with **CNT-165**, the defined term, in T6                                                                                                                                          |
| Rendering every character in output needs faces for every script, which is T6                                                                                                                     | **CNT-055 superseded by CNT-166** - storable and shown in the editor, and a character a publication's typefaces cannot set failing the publish by name. Rendering in output beyond the pinned faces is already **STY-075**, in T6, so no row is made for it |
| Keeping a Word heading as a heading means breaking the paste into sections and components, which is Word import and breakout, T6                                                                  | **CNT-060 superseded by CNT-167** - structure kept, a heading made a paragraph and named in the paste report, T1 - with **CNT-168**, the heading kept, in T6                                                                                                |
| Reaching back past the version the session opened from is restoring, an audited act, and the audit log is T3                                                                                      | **CNT-103 superseded by CNT-169** - undo never past the opening version or a version cut, T1 - with **CNT-170**, restoring as the separate and audited act, in T3                                                                                           |

| Counts           | Before                      | After                       |
| ---------------- | --------------------------- | --------------------------- |
| Requirements     | 153, of which 16 superseded | 170, of which 25 superseded |
| Non-requirements | 10                          | 10                          |
| Open questions   | 15, of which 9 settled      | 15, of which 9 settled      |
