...
| **CNT-Q11** | **Which area owns a document being created from a template?** Filling parameters, resolving variables, running the queries a template declares, materialising the outline. §6 defines a parameter set and §7.3 covers bulk generation, but no area owns single-document instantiation - and §5 names a template designer persona no area serves                                           | A decision on whether to add a nineteenth area (`TPL`), covering template authoring, definition binding and instantiation. Same shape as the gap `STY` just filled                                                                                                                  |
