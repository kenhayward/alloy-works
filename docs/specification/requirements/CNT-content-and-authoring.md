...
**SCH** searches the values. A component's own title and the attributes every component carries are
CNT-142 to CNT-144; what a component's _type_ declares is still open, in **CNT-Q12**.
...
| **CNT-142** | A component must carry a title of its own, distinct from the title of any outline section that places it (**STR-005**), so that it can be listed and found in a space holding no documents at all                                                                                              | T1         | Specified |
...
**CNT-144 says what is deliberately not here.** A component is also **typed**, and a type declares
the metadata its components carry - which is a subsystem rather than a row, and is specified
separately. The set being closed is what stops a field arriving in the schema by accident, which is
the failure **CNT-Q12** named.
...
| **CNT-Q12** | **Who owns a component's own metadata - its title, its tags?** **TPL** owns the metadata schema a document declares and **SCH** searches the values, but no area states where a component's own title and values live, or what validates them                                                                                                                                             | **Half settled.** The structural half is here: CNT-142 to CNT-144 give a component its own title and close the set of attributes every component carries. The typed half is still open - a component is _typed_, and a type declaring the metadata its components carry is a subsystem the size of TPL's metadata half. What would settle it: a decision on where component types live, given that a document's schema comes from its template but a component is transcluded into many documents and so cannot take its type from any of them |
