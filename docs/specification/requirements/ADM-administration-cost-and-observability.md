...
includes the declared rate limits of **API-051**; there is no separate administrative back door for a
limit to miss.
...
| **ADM-040** | Scheduled and system work that affects a tenant - a review date falling due (**LIF-060**), a budget evaluation, an external source re-check (**LIB-035**), a translation job, indexing, a derivative rebuild, a scan timeout, a cohort run - must be visible to that tenant with its state, when it last ran, when it is next due and why it last failed. Work a user submits follows the job contract in **API-040** to **API-043**; this is the rest | T3      | Specified |
| **ADM-041** | A tenant must be able to see one declaration of everything that can leave its boundary: model endpoints (**GEN-031**), embedding (**GEN-038**), external reference sources (**LIB-037**), translation services (**LOC-026**), malware scanning (**AST-Q03**), data connections, webhooks and notification delivery. For each it must state what may leave, whether it is on by default, who may change it, and any residency constraint (**ADM-Q01**)  | T3      | Specified |
| **ADM-042** | A tenant administrator must be able to grant, review and revoke support access (ADM-023) as an administrative act: each grant declaring its scope - metadata only, diagnostics, content read - its expiry, its purpose and who it is for, audited into the tenant's own log and shown in ADM-025. Support access must appear in the effective-permission view (**IAM-029**, **IAM-030**) like any other grant                                          | T3      | Specified |
| **ADM-043** | Backup frequency, recovery point and recovery time must be declared per artifact class and tested: restoring must be exercised on a stated schedule rather than assumed, and a restore that is never rehearsed must not be presented as a capability (ADM-020)                                                                                                                                                                                         | T3      | Specified |
| **ADM-044** | Stored artifacts must have declared integrity re-verification schedules - assets against their hashes (**AST-042**), version digests (**VER-042**), audit exports (**LIF-055**) - and a verification failure must be surfaced in diagnostics and audited rather than logged                                                                                                                                                                            | T3      | Specified |
| **ADM-046** | Every performance budget in the specification must be registered with its owner, its reference configuration, whether it is provisional, the tranche by which it is confirmed, and how it is measured in production. ADM-016's reporting must say, budget by budget, whether it is being met                                                                                                                                                           | T3      | Specified |
| **ADM-048** | Cost and usage attribution (ADM-010, ADM-011) must extend to every chargeable external operation: external reference searches, translation jobs, malware scanning, notification and email delivery, webhook delivery, indexing and embedding, and bulk generation                                                                                                                                                                                      | T5      | Specified |
...
| **ADM-031** | Suspension and closure must each declare in advance which capabilities stop and which continue - in-flight publications, scheduled work, webhook delivery - as ADM-014 does for an exhausted budget                                                                                                  | T3      | Superseded by ADM-045 |
| **ADM-045** | Suspension and closure must each declare what stops and what continues, naming at least: shared publication links (**PUB-081**), webhook delivery, scheduled and background work (ADM-040), realtime connections, model endpoints, search indexes, notification delivery, and in-flight publications | T3      | Specified             |
...
**ADM-040 names the layer that nine documents assume and none owned.** Review dates falling due,
budget alerts, source re-checks, translation jobs, indexing, derivative rebuilds, scan timeouts and
cohort runs are all background work somebody has to run, and **API-040** to **API-043** cover only
what a caller submitted. Making the rest visible per tenant - state, last run, next due, last failure

- is what stops "it should have happened by now" being a support conversation.
...
| **ADM-N02** | **No cross-tenant reporting** of content, or of product analytics identifying individuals or unrelated tenants, other than aggregates that identify nobody. Administrative and cost reporting across the tenants of one customer organisation is permitted to that organisation's own administrators (**IAM-068**, **IAM-069**), because they are one customer looking at themselves |
...
| **ADM-Q07** | **Should notifications, background work, and security and data protection become areas of their own?** Each is now specified as cross-cutting requirements with named owners - COL-060 to COL-062 and API-055 for notification and webhook mechanics, ADM-040 for scheduled work, ADM-041 to ADM-044 and IAM-066 for boundaries, backup and derived copies - which is the smaller move. Each could instead be an area | Whether the cross-cutting requirements hold together as the tranches are planned. An area earns its own document when its requirements stop fitting in somebody else's, and these are close to that line rather than over it                     |
...
| ADM-003            | API-004, administration on the same terms as the interface; API-051, rate limits declared in the specification |
...
| Throttling of the administrative API    | **Confirmed by construction.** ADM-003 puts administration on the same terms as the interface, so the declared rate limits apply to it (**API-051**, which superseded API-009); there is no separate administrative surface for a limit to miss. Boundary row added         |
...
| ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.5, 2.11 to 2.14, 2.16, 2.18 to 2.22, 2.26 | **ADM-040** makes scheduled and system work visible per tenant - the layer nine documents assumed and none owned. **ADM-041** gives a tenant one declaration of everything that can leave its boundary. **ADM-042** turns support access from a principle into an administrative control. **ADM-043** and **ADM-044** cover backup, restore rehearsal and integrity re-verification. **ADM-031 superseded by ADM-045**, naming what stops at suspension and closure. **ADM-046** registers every performance budget with an owner and a confirmation tranche. **ADM-032 superseded by ADM-047**, covering every outbound credential rather than three kinds. **ADM-048** extends cost attribution to every chargeable external operation. **ADM-N02** now admits organisation-level administrative and cost reporting to that organisation's own administrators. **ADM-Q07** records whether notifications, background work and data protection should become areas. Rate-limit citations move from API-009 to **API-051** |
