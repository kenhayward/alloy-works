...
| Finding a section to reuse from          | **SCH** |
...
| **REU-054** | An author must be able to place the components a section in another document places into their own document in one act, as references under a new section of their own. The found section must not be shared (**STR-004**); the new section must take its fields from the author's own template (**STR-060**) and copy no values; each reference must take a mode REU-050 allows; and REU-046's report must run before the act is accepted | T4         | Specified             |
...
**REU-054 is how a section in somebody else's document becomes reusable without becoming shared.**
STR-004 forbids sharing a section, and rightly: a section is position, and position belongs to one
document. What an author wants from a well-built section elsewhere is its components, in its order,
under a heading of their own. Placing those as references gives exactly that and leaves the found
document untouched. The section's own field values are not copied, because they answered the other
document's template, and the new section answers this one's. Finding the section is SCH-053's.
...
Not a review. Specifying [MET](MET-metadata-and-component-types.md) gave sections metadata so that
components could be found through the sections that place them, and reusing them as a group had no
requirement.

| What was found                                                                                                                                                                                                               | Change                                                                                                                             |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Reuse was specified one component at a time (REU-001), and STR-004 forbids sharing a section - so reusing what a section in another document holds had no stated act, and the obvious implementation would share the section | **REU-054**: place the found section's components as references under a new section of the author's own, copying no section values |

| Counts           | Before                    | After                     |
| ---------------- | ------------------------- | ------------------------- |
| Requirements     | 53, of which 3 superseded | 54, of which 3 superseded |
