...
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | --------------------- |
| **PUB-001** | Publishing must run as four named stages - resolve, compose, paginate, render - each able to fail with diagnostics naming the stage                                                                                        | T1         | Superseded by PUB-086 |
| **PUB-086** | Every publishing failure must name the stage it arose in - resolving the document, composing it against its layout and theme, the engine, or storing the output - and the node, block, reference or definition it concerns | T1         | Specified             |
...
| **PUB-008** | A layout must declare running heads and feet, with fields for at least the document title, the current section, the page number, the physical page count and the revision                      | T1      | Specified             |
...
| ID          | Requirement                                                                                                                                                                                                                                                                                        | Tranche    | Status                |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | --------------------- |
| **PUB-015** | PDF must be a first-class output and must meet the fidelity bar in scope §4                                                                                                                                                                                                                        | T1         | Superseded by PUB-087 |
| **PUB-087** | PDF output must pass the publishing regression corpus - the engine spike's cases, grown by a case for every publishing defect found - on every change to the engine, the template or the pipeline                                                                                                  | T1         | Specified             |
...
| **PUB-096** | A cross-reference in a paragraph's text - running text, a list, a quotation, a table's body cell, a footnote's text - must be an internal link to its target                                                                                                                                       | T1         | Specified             |
...
that claim inspectable a year later, when somebody asks. PUB-091 now carries it, naming the checker.

**PUB-090 splits PDF/UA into what a machine can check and what a person must.** veraPDF checks the
machine-checkable rules of PDF/UA-1 - the only part the engine writes - on every publication. The
...
| **PUB-064** | A 300-page document must publish in under thirty seconds - a provisional budget, to be confirmed against real content                                                                                                                                                                                        | T1         | Superseded by PUB-085 |
| **PUB-085** | A declared 300-page reference document - prose, figures, tables, equations and footnotes - must publish to PDF, from the request to the recorded publication, at or under ten seconds at p95 with no measured sample above thirty, measured on a declared reference configuration recorded beside the result | T1         | Specified             |
...
| **PUB-082** | A publication's record must be no weaker than the baseline's pins (VER-018, **VER-055**) and must additionally identify any accepted generated content it contains, with the prompt, model and context digest retained for it (**GEN-023**, **GEN-046**)                                                     | T3         | Specified             |
...
| **PUB-094** | Publishing must be refused where the publisher may not read every component the resolved document contains, naming each such place in the outline and never the component; a publication must never contain what its publisher could not read                                                                                                                                                                                                                                                                       | T1         | Specified             |
| **PUB-079** | A resolved document in which no outline node survives conditions must publish the front and back matter its layout declares that has something to show - a cover, or a contents with entries - and must fail with a named error otherwise - never produce an empty artifact                                                                                                                                                                                                                                         | T1         | Specified             |
...
Not a review. [The publishing design](../../design/publishing.md) challenged eight of this area's
requirements and proposed three more; Ken accepted every challenge. The three proposals were filed as
issues [#142](https://github.com/kenhayward/alloy-works/issues/142),
[#143](https://github.com/kenhayward/alloy-works/issues/143) and
...
the requirement Ken filed from the publishing design for the layout, and reworded two it cites.
...
| PUB-079 said "no body content", which the outline does not define, and failed "only where the layout declares none", which left a layout declaring only a contents, with nothing to put in it, answered two ways. Ken's answer, 2026-09-19: refuse it | **PUB-079** reworded for clarity, identifier kept: where no outline node survives conditions, the front and back matter the layout declares that has something to show - a cover, or a contents with entries - is published, and anything else fails by name |
| PUB-008 said "the total pages", which a document numbered per matter (front matter in roman, the body from 1) leaves open: the pages of the whole PDF, or of the page's matter. Ken's answer, 2026-09-19: the whole PDF's, as decision I measured     | **PUB-008** reworded for clarity, identifier kept: "the total pages" became "the physical page count"                                                                                                                                                        |
...
| PUB-022 asked for every cross-reference to be a link, but structure.md's decision XR-D sets one outside a paragraph's text as text: measured, a link in a running head refuses the compile, and one in a caption or a title nests inside the entry the lists and the contents set for it | **PUB-096** asks for a link only in a paragraph's text. A reference in a caption, a section's title, a definition term, an attribution, a header row or a table's note is set as text, by XR-D, and no requirement asks for it to be a link |
