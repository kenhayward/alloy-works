...
| **IAM-005** | Search indexes, caches, secrets, model endpoints, publications and the audit log must each be tenant-scoped, not only the content                                                                                                                                                                                                                                                                                                                                                   | T1         | Superseded by IAM-075 |
| **IAM-075** | Search indexes, caches, secrets and publications must each be tenant-scoped, not only the content                                                                                                                                                                                                                                                                                                                                                                                   | T1         | Specified             |
| **IAM-076** | Model endpoints must be tenant-scoped                                                                                                                                                                                                                                                                                                                                                                                                                                               | T5         | Specified             |
| **IAM-077** | The audit log must be tenant-scoped                                                                                                                                                                                                                                                                                                                                                                                                                                                 | T3         | Specified             |
| **IAM-066** | The lists in IAM-075, IAM-076 and IAM-077 must be read as examples of one rule: every second copy, derived representation or outbound payload of tenant content must be tenant-scoped and permissioned like the content it derives from. That includes search indexes, caches, embeddings and vectors, asset derivatives, export artifacts, notification messages and digests, webhook payloads, audit exports and backups, and every path must be covered by a test like IAM-004's | Constraint | Specified             |
...
| **IAM-052** | A customer must be able to hold several tenants - production, a sandbox, a validation environment - grouped by an organisation that shares billing, administration and identity provider configuration, and never content                                                                                                                                                                                                                                                           | T1         | Superseded by IAM-078 |
| **IAM-078** | A customer must be able to hold several tenants - production, a sandbox, a validation environment - grouped by an organisation that holds no content                                                                                                                                                                                                                                                                                                                                | T1         | Specified             |
| **IAM-079** | An organisation must share billing, administration and identity-provider configuration across its tenants (**IAM-068**)                                                                                                                                                                                                                                                                                                                                                             | T3         | Specified             |
...
| **IAM-010** | A user disabled at the identity provider must lose access without waiting for a token to expire                                                                                                                                                                                                                                                                                                                                              | T2         | Specified |
...
| **IAM-013** | Every authentication, and every authorisation that resulted in a refusal, must be recorded in the audit log (**LIF** owns the log)                                                                                                                                                                                                                                                                                                           | T3         | Specified |
...
| **IAM-060** | Bootstrapping a tenant must be audited into that tenant's own log: who invited the first administrator, when, under what authority, and when the vendor's part in it ended                                                                                                                                                                                                                                                                   | T3         | Specified |
...
| **IAM-023** | The modes of access in CNT-154 and CNT-155 must derive from these permissions, so that a mode a user cannot have is a mode they are never offered                                                                                                                                            | T1         | Specified |
...
| **IAM-037** | Token issue, use and revocation must be recorded in the audit log                                                                      | T3         | Specified |
...
| What was found                                                                                                                                | Change                                                                                                                                                                     |
| --------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IAM-013 records into the audit log, which **LIF** owns and the scope puts in T3                                                               | **IAM-013 moved to T3**                                                                                                                                                    |
| IAM-037 needs the audit log, which is T3                                                                                                      | **IAM-037 moved to T3**                                                                                                                                                    |
| "Audited into that tenant's own log" needs the audit log, which is T3                                                                         | **IAM-060 moved to T3**                                                                                                                                                    |
| Learning of a disablement needs SCIM (IAM-008) or re-reading the provider (IAM-056), both T2                                                  | **IAM-010 moved to T2**                                                                                                                                                    |
| Model endpoints arrive with **GEN** in T5, and the audit log in T3                                                                            | **IAM-005 superseded by IAM-075** - search indexes, caches, secrets and publications, T1 - with **IAM-076**, model endpoints, in T5, and **IAM-077**, the audit log, in T3 |
| Sharing billing, administration and provider configuration across tenants needs the organisation's own administrators (IAM-068), which are T3 | **IAM-052 superseded by IAM-078** - several tenants grouped by an organisation holding no content, T1 - with **IAM-079**, what the organisation shares, in T3              |
