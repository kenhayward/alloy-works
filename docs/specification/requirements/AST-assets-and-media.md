...
| **AST-051** | Each format admitted as an asset must declare how an upload of it is made safe before anything can reference it: scanned for malware, or proved by a strict parse of its structure and a full decode to hold nothing but content of that format. A format that can carry active content - script, links, or embedded files, such as SVG, PDF or an Office document - must be scanned | Constraint | Specified             |
