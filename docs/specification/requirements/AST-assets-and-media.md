...
| **AST-003** | An upload must be scanned for malware before it can be referenced by anything                                                                                                                                                                                                                                                                                                                                                                  | Constraint | Superseded by AST-051 |
...
| **AST-035** | An upload awaiting its check (AST-051) must be in a named quarantined state, visible to the uploader, and must not be referenceable, listable as available, or usable by anything until it clears                                                                                                                                                                                                                                              | Constraint | Specified             |
...
| **AST-037** | An upload that fails its check (AST-051) must be refused, must never become referenceable, and the refusal must be audited without retaining the file anywhere content could reach it                                                                                                                                                                                                                                                          | Constraint | Specified             |
| **AST-051** | Each format admitted as an asset must declare how an upload of it is made safe before anything can reference it: scanned for malware, or proved by a strict parse of its structure and a full decode to be a well-formed file of that format, with nothing that follows its end kept. A format that can carry active content - script, links or embedded files a reader would act on, such as SVG, PDF or an Office document - must be scanned | Constraint | Specified             |
...
**AST-051 supersedes AST-003, which put a malware scanner in front of every format.** A signature
scan of a PNG or a JPEG finds very little: the harm either carries is a decoder exploit or a second
file hidden after the image, which a strict walk of its structure and a full decode refuse outright,
and a scanner costs every deployment gigabytes of memory and a line to the internet. So each format
says how it is made safe, and the formats that can carry active content are still scanned
([the figures design](../../design/assets.md#requirements-challenged), issue #206).

**AST-035 to AST-037 name the state an asset was already in and nobody had written down.** AST-051
says an upload cannot be referenced until it has been checked, which means there is a period where
...
| ID          | Requirement                                                                                                                                                                                                                                        | Tranche    | Status    |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | --------- |
| **AST-016** | Replacing an asset must create a new version rather than overwriting it (**VER-056**)                                                                                                                                                              | Constraint | Specified |
...
### A format made safe by proof, 2026-09-23

Not a review. The figures design found AST-003's scan costing every deployment a malware scanner
before the first image, for two formats a scanner can say little about
([assets.md](../../design/assets.md#requirements-challenged)); Ken took decision F-A as recommended,
and it was filed as issue #206. The row's wording is narrower than the issue's: the figures 1 review
found that a parse and a decode prove a file is well formed and keep nothing after its end, but not
that no metadata inside it carries other bytes - a zip in a PNG text chunk is still a well-formed PNG.
What metadata an asset keeps is AST-007's declared policy, T2's, and the row says only what is proved.

| What was found                                                                   | Change                                                                                                                                                |
| -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| AST-003 required a malware scan of every upload, whatever its format could carry | **AST-003 superseded by AST-051**: each format declares how it is made safe, scanned or proved by parsing and decoding, and active content is scanned |
| AST-035 and AST-037 pointed at AST-003's scan                                    | Both now read "its check (AST-051)" - a pointer repointed, since a scan is still one way a check is made. No row was added for them                   |
| AST-036 and AST-Q03 speak of a scan                                              | Unchanged: both still apply to the formats AST-051 requires scanned, and none of those is admitted yet                                                |
