...
| [MET](MET-metadata-and-component-types.md)          | Fields and metadata schemas, which a type assigns  |
...
| Fields and metadata schemas themselves      | **MET**          |
...
| **REL-003** | A type must be able to declare a metadata shape that instances of it carry                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | T4         | Superseded by REL-053 |
| **REL-053** | A relationship type must be able to assign metadata schemas (**MET-005**), whose fields its relationships carry. A relationship type is one of the places schemas apply (**MET-034**)                                                                                                                                                                                                                                                                                                                                                                        | T4         | Specified             |
...
| **REL-036** | Changing a type's metadata shape must be guarded exactly as its endpoints and cardinality are (REL-006): adding a required field, narrowing a field's type or removing one must be refused where existing instances would become invalid, and the refusal must say how many would                                                                                                                                                                                                                                                                            | Constraint | Superseded by REL-054 |
| **REL-054** | A change that would make an existing relationship invalid must be refused, saying how many would become invalid: changing a type's endpoints or cardinality (REL-006), assigning a schema to a type or removing one, and a new version of a schema or field the type assigns (**MET-035**) that adds a required field, narrows a field's type or validation, or removes a field. A relationship is edited in place rather than versioned (REL-033), so it cannot be judged by the definitions it was written against as a component version is (**MET-017**) | Constraint | Specified             |
...
**REL-053 and REL-054 bring a relationship's metadata into MET.** REL-003 let a type declare "a
metadata shape" of its own - a second schema system beside the one templates and component types use,
with fields that could never facet alongside anybody else's. A type now assigns metadata schemas, so
`effective date` on a supersedes relationship is the same field as `effective date` on a component.

**The rule for changing definitions stays REL's, and it is deliberately not MET's.** A component version
is immutable, so it is judged for ever by the definitions it was written against (MET-017) and a
definition change can never break it. A relationship is edited in place, so it has nothing recorded to
be judged by, and a change that would break one has to be refused instead. What is new is that the
change can arrive from a shared schema or field rather than from the type - so a schema version can now
be refused because of relationships, which MET-035 names as one of the places it checks.
...
Not a review. Specifying [MET](MET-metadata-and-component-types.md) found a second schema system here.

| What was found                                                                                                                                                                                              | Change                                                                                                                                                                     |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REL-003 gave a relationship type "a metadata shape" of its own, beside the metadata schemas templates and component types use - two definitions of the same idea, with fields that could not facet together | **REL-003 superseded by REL-053**: a type assigns metadata schemas                                                                                                         |
| REL-036 guarded changes to a type's metadata shape. With shared schemas and fields, a relationship can be broken by a change to a schema or a field, not only to its type                                   | **REL-036 superseded by REL-054**, which guards all of those. The guard itself is unchanged in kind - refuse, and say how many - because relationships are edited in place |
| Not changed: **REL-010, REL-033, REL-034 and REL-051**                                                                                                                                                      | Each speaks of "the metadata its type declares" or filtering by it, which is still true with the fields now coming through assigned schemas                                |

| Counts           | Before                    | After                     |
| ---------------- | ------------------------- | ------------------------- |
| Requirements     | 52, of which 3 superseded | 54, of which 5 superseded |
| Non-requirements | 5                         | 5                         |
| Open questions   | 6, of which 4 settled     | 6, of which 4 settled     |
