...
**The panel does not offer front matter, and does not number with the layout's scheme.** The outline
holds `front` as a third matter since schema 2, and `POST /v1/documents/{id}/outline` sets it, so
front matter is reachable through the API alone; the panel shows **Numbered** and **Appendix** and
numbers with the product's default scheme, not the document's layout's. Both were planned with the
layout and left unbuilt, and publishing's third slice owes them - see
[`design/publishing.md`](design/publishing.md), "The layout". An operation that would break the
front-first order is refused by `applyOutlineOperation` before the parse, with **Front matter comes
before the rest of the outline**, which the service carries through as the `outline_invalid` reason
and the page shows as the panel's notice.
