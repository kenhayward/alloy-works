...
> its number beside it, which a cross-reference can point at and a layout can list. Nothing yet
...
the maths face - described in its bullet below.
...
item but text. The theme, a list of equations in the default layout, veraPDF on every publication, preview and Word are later slices'. Nothing chooses or edits a
...
- **The pinned faces.** `apps/worker/fonts/` holds Liberation Serif 2.1.5 and, from editor 5,
  ...
  file from the tag `v2.13b171` of stipub/stixfonts, one family per `Face` (`body`, `code` or `math`),
  each under the SIL Open Font Licence with its text beside it (`LICENSE-Liberation.txt`,
...
- **The publication templates.** `apps/worker/templates/publication/1/main.typ` reads `assemble`'s
  ...
  `template.ts` and annotated with its literal, `'publishing/11'`, so repointing `PUBLISHING_SCHEMA`
  fails to compile there, and a frozen schema's row by its own frozen constant; `satisfies` refuses a
  row for a schema nothing makes, `publishing/9`'s and `publishing/10`'s excepted, which are kept. A version is immutable:
...
- **Equations (`publishing/11`, template 11).** `packages/domain/src/publishing/maths.ts` is the one
  ...
  per column, phantoms, and display and inline style - with its alternative, the root's `alttext`, or
  a refusal naming the construct. It keeps an allowlist of its own rather than the reader's, so an
  element or an attribute the mapping does not know is refused, never dropped; `\scriptstyle` and a
  column aligned two ways are refused, and an operator's `minsize` and `maxsize`, `mpadded`'s sizes,
  ...
  bookmarks follow it. A block equation wider than its line still runs past the margins, and nothing
  refuses one. `PIPELINE_VERSION` is `'11'`. `apps/worker/src/equations.test.ts` is the regression
