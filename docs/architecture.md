...
> or a footnote from a dialog and shows it as it will print, or as broken, refused by name at publish
> until cross-references 2. Nothing yet pastes an image from outside the product, edits or
> publishes an equation, makes a component type or resolves a cross-reference at publish; an administrator
...
in its text and in the editor opened in place; nothing resolves a cross-reference at publish or shows a
document as a document view.
...
| `domain: structure/references.ts`                         | `documentTargets`, in document order, every section as a `node` target and every figure, table and footnote an occurrence contributes - a `block` target in the occurrence being edited, a `component` target where its component occurs once, and nothing where it occurs more than once - each with its label, title and whether it is above or below; `formsFor`, the forms a kind of target has; `printed`, what a form prints, in English until the layout's words arrive; and `kindWord`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
