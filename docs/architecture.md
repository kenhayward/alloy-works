...
> read the space, though nothing in the editor places one yet. Nothing yet edits, pastes or publishes a
> figure, a footnote or an equation, makes a component type or resolves a cross-reference; an administrator
...
answered by the code-aware commands ahead of the list's in preformatted text); one holding anything
else opens for reading only, and nothing recovers an iteration or edits metadata. A paste reaches
...
editor uses it yet: the routes are the whole of it.

| Where                               | Holds                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `domain: src/assets/header.ts`      | `ADMITTED_FORMATS` - PNG and JPEG, each made safe by proof (AST-051) - the limits, 25 MB and 50 million pixels, and `readImageHeader`: a strict walk of a PNG's chunks by their lengths and CRCs to `IEND`, or a JPEG's markers and entropy-coded data to `EOI`, refusing a byte after the end, a coding the pipeline does not decode, and more pixels than the limit on the header alone. Dimensions are recorded as displayed, turned by the EXIF orientation |
...
| `db: src/assets.ts`                 | `createAssetUpload`, `receiveAssetBytes` (the move to checking and the `ingest` job, in one transaction), `recordAsset` (the asset in the upload's space at 0.1 by its uploader, and the upload ready, in one), `refuseAssetUpload`, `readAssetUpload`, `readAssetVersion` and `objectNamedByAsset`                                                                                                                                                             |
...
| `service: src/assets.ts`            | The handlers, and the service's half of the check: the format from the bytes, the walk and the pixel limit, each refusal committed on the upload before it is answered; then the bytes kept under their hash                                                                                                                                                                                                                                                    |
| `worker: src/jobs/ingest.ts`        | The job: the bytes read back and held to their key's hash, walked again, decoded whole by sharp 0.35.4 under the pixel limit, the two readings compared, then `recordAsset` - or the upload refused and its bytes removed unless an asset already holds the same ones. Its last failure refuses the upload as `unchecked`                                                                                                                                       |
