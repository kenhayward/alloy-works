...
| `domain: structure/contributions.ts`                      | `contributionsOf`, a component version's content projected to what it contributes: each figure, table, block equation and footnote, in document order wherever it is nested, and whether it is numbered                                                                                                                                                                                                                                                                                                                                                                   |
...
| `api-contract: documents.ts`                              | `GET /v1/documents`, `POST /v1/spaces/{space}/documents`, `GET /v1/documents/{id}`, `GET /v1/documents/{id}/numbering` and `POST /v1/documents/{id}/outline`, and their schemas; the operation's schema is the domain's, and the service validates a body with it, so what `openapi.json` cannot express - that only a page reference carries `withoutPages`, and that a title's cross-reference targets an outline node alone - is refused at runtime as `invalid_request`                                                                                               |
| `service: src/documents.ts`                               | The handlers: a document's kind checked by each, a stale act answered with the current outline, `outline.invalid` mapped to `outline_invalid`, every answer carrying an outline built through one view that withholds what the caller may not read, and the numbering of the latest version, numbered against the default scheme from `numberingInputs`                                                                                                                                                                                                                   |
| `web: src/structure/`                                     | The documents list, **New document**, the document page holding the outline the last act returned and an undo stack, the outline panel with its keymap and drag and drop, each node's section number computed with `number` on every render and shown as the item's description, **Numbered** and **Appendix** beside the selected node, and `tree.ts`, the panel's arithmetic, pure and tested against the domain's operations                                                                                                                                           |
