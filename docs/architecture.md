...
> a page or above and below - as a link in a paragraph's text. Nothing yet pastes an image from
> outside the product, edits or publishes an equation or makes a component type; an administrator
...
(footnotes 1) and cross-references (cross-references 1: a `crossReference` atom placed and changed from
the **Reference** dialog, showing what it will print); one
holding anything else opens for reading only, and nothing recovers an iteration or edits metadata. A paste reaches
...
| `editor: src/marks.ts`                                              | `EDITOR_COMMANDS`, the one registry the toolbar and the keymap both read, nineteen rows of two kinds, **Reference** the last; `commandKeymap`; `toggleMarkCommand`, `applyMarkCommand` and `removeMarkCommand`; and `markAt`, `markThroughout` and `somewhereToPutMark`, which a renderer asks before it asks the author for anything                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `editor: src/blocks.ts`                                             | `blockCommand` over ten `BlockAction`s, **Reference** the tenth - which answers whether one could be placed and places nothing, since its dialog places it - `listAwareEnter`, `setListAttributes` and `listAt` - the list commands, the Enter chain and the one query the list panel reads                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `web: src/editor/`                                                  | The session as a state machine over a service and a clock, the adapter onto the generated client, the save indicator, the editor and its header, the **Formatting** toolbar, the mark prompt and the **List** panel, the **Figure** and **Image** buttons, the dialog, the panel - for a figure or an image selected whole - `ReferenceDialog` and `referenceChoices.ts`, what it offers and what each choice will show, and `uploadImage` - the two requests of an upload, then the upload followed every half second for thirty seconds, and a refusal said in words - `press.ts` - the one function a button press and a shortcut both run - **New component**, the list and the workspace                                                                                                                                                                                                                                                                      |
...
nineteen rows of two kinds - nine marks and ten block actions - each with the label, the key, how the
...
| `domain: publishing/layout.ts`                             | The layout's stored shape at schema 3, strict at every depth, with `matter.lists` - the generated lists it declares, each sequence once, each with its title - and `words.above` and `words.below`, both or neither, the words a relative cross-reference prints; `layoutWordsSchema`, the words alone; `parseLayout`, `readLayout` and its migration chain, which reads a schema 1 layout as declaring no lists and a schema 2 one as having no words for above and below; `FIRST_DEFAULT_LAYOUT`, 0018's literal, `SECOND_DEFAULT_LAYOUT`, 0019's, and `THIRD_DEFAULT_LAYOUT`, 0021's, all frozen, the last two at schema 2 and read through today's chain when the module loads, and `defaultLayout`, 0023's; `speaksFor` (RFC 4647 basic filtering) and `unsupportedFormats` |
...
- **The pinned faces.** `apps/worker/fonts/` holds Liberation Serif 2.1.5 and, from editor 5,
  Liberation Mono 2.1.5 - four faces of each, one family per `Face` (`body` or `code`) - committed
  and pinned by
...
- **The publication templates.** `apps/worker/templates/publication/1/main.typ` reads `assemble`'s
  ...
  `template.ts` and annotated with its literal, `'publishing/10'`, so repointing `PUBLISHING_SCHEMA`
  fails to compile there, and a frozen schema's row by its own frozen constant; `satisfies` refuses a
  row for a schema nothing makes, `publishing/9`'s excepted, which is kept. A version is immutable:
...
- **Cross-references (`publishing/10`, template 10).** `assemble` resolves every cross-reference in
  ...
  `{ reference: { anchor, text, page, link } }`, a link only in a paragraph's text, a footnote's
  included; a section's title prints its own reference as its number, in its words. Every published
  block, footnote and node carries `anchor`, straight after `id` - `b-<node>-<block>` or `n-<node>` -
  and null unless a reference names it, and a named block that publishes nothing leaves a
  `PublishedMarker`, `{ type: 'marker', anchor }`, in its place. `cross_reference_unresolved` names the
  ...
  `page-numbering`, wrapped in `link(label(anchor), ..)` only where `link` is true - and a label, built
  with `label(..)`, on every element carrying an anchor: a heading, a paragraph, a list, preformatted
  text, a quotation, a table's figure, a figure, a footnote and a marker, set as `metadata(none)`;
  ...
  as _A cross-reference cannot be published under this request's layout._
...
- **Images in a line of text (`publishing/8`, template 8).** A published run may also be a
  ...
  (`INLINE_IMAGE_HEIGHT`) with its width from its proportions, refuses one wider than the room where it
  stands (`image_too_wide`) - in a table's cell, the cell's share of the measure less the engine's
  inset of 5 points each side (`cellIndent`) - and describes and refuses it as a figure, naming the
...
- **What a test can see of a PDF.** `apps/worker/src/testing/pdf.ts` reads a compiled PDF back:
  ...
  objects themselves - and `languages`, each declaring run's language paired with the text it covers. That
