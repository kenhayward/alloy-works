...
published, and every other inline item - an image, a footnote, a cross-reference and an equation - each set from the environment's theme, tables and images from their table and image styles. A Word document carries a document's paragraphs, headings, marks, links and languages, its lists, quotations, preformatted text, tables, figures and images in a line, its footnotes as Word's own, its cross-references as fields Word updates and its equations as Word's own, OMML from the maths tree the PDF sets, and its page - the cover, the contents, the lists after the contents, the running heads and feet and the notice - and refuses by name what Word would print otherwise than the PDF: a number Word's fields cannot compute (`numbering_not_in_word`, `list_not_in_word`) and a reference whose field would print other words (`cross_reference_not_in_word`); a heading or a listed caption whose equation Word's rebuilt entries set as its runs alone it reports, and a maths character Cambria Math lacks it neither refuses nor reports. A list of equations in the default layout, veraPDF on every publication, preview and review in Word are later slices'. Nothing chooses or edits a
