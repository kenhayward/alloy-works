...
> of figures and tables after the contents, or a Word document of all of that but its footnotes,
> cross-references and equations, or both, keeps it and lists it - and [assets](#assets), which takes an
...
preformatted text, tables, figures and images into Word, designed in
...
published, and every other inline item - an image, a footnote, a cross-reference and an equation - each set from the environment's theme, tables and images from their table and image styles. A Word document carries a document's paragraphs, headings, marks, links and languages, its lists, quotations, preformatted text, tables, figures and images in a line, and its page - the cover, the contents, the lists of figures and tables, the running heads and feet and the notice - and refuses footnotes, cross-references, equations and the list of equations by name (`word_not_yet`) until later slices write them. A list of equations in the default layout, veraPDF on every publication, preview and the rest of Word are later slices'. Nothing chooses or edits a
...
- **The `publish` job.** `apps/worker/src/jobs/publish.ts` reads exactly what the request recorded
  ...
  document places by their paths, kept in the store as a `.docx` by its hash and recorded as `word/2`
...
- **`assemble` told its formats, and what it answers for Word** (Word 1). `AssembleInput.formats` is
  ...
  row, a missing continuation label's words, and the maths tree's reasons but `unreadable`, `merror`
  and `empty` - are tagged as they are made and said only where `pdf` is asked for, and since Word 2
  none of them drops its construct from the published document but a footnote in a header row, a
  reference to a header-row target and an equation the maths tree refuses, each still refused for
  Word. Where `docx` is, `word_not_yet` names each construct the writer does not write yet - a block
  equation, and in a line or a title an equation, a footnote or a cross-reference, and the list of
  equations - `numbering_not_in_word` a heading's, a figure's or a table's number Word's fields
  would compute otherwise, `list_not_in_word` a list past the ninth level, in letters from the 28th
  or past roman 3999, and `image_too_wide` an image in a line wider than Word's line;
  `format_unsupported` is said, not thrown, for Word under a layout with no Word page. The answer
  carries `word: WordInput` - the layout's `docx` member, the resolved theme, the scheme, and each
  figure's and inline image's size against the Word page, by the PDF's own functions, keyed by
  `figureImageKey` and `inlineImageKey` - exactly where Word is asked for, and null otherwise. The
...
faces, images })`, pure and deterministic - a fixed zip time, the parts in a fixed order, relationships
  ...
  [`design/word-output.md`](design/word-output.md#what-was-built) says what Word 1 and Word 2 built
