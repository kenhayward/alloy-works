...
pipeline first needed it: the stored shapes `catalogue/1` and `theme/1`, closed at every depth; the
reader, `readCatalogue` and `readTheme`, which walks every `basedOn` chain, finds every typeface,
place and role, checks each place's and role's style applies there and each colour's contrast, and
returns every refusal at once; the default theme, `DEFAULT_THEME`, and its catalogues under fixed
version identifiers; and three projections - `projectTypst`, the data template 12 reads, which
`assemble` uses, and `projectCss` and `projectStylesXml`, which nothing calls yet and which leave out
...
published, and every other inline item - an image, a footnote, a cross-reference and an equation - each set from the environment's theme. Table and image styles with properties of their own, a list of equations in the default layout, veraPDF on every publication, preview and Word are later slices'. Nothing chooses or edits a
...
| `domain: theme/`                                           | The theme model (see [workspaces](#workspaces)): `catalogue/1` and `theme/1`, `readCatalogue` and `readTheme`, contrast, `DEFAULT_THEME`, `DEFAULT_CATALOGUES` and `DEFAULT_CATALOGUE_VERSIONS`, and `projectTypst`, which `assemble` puts in `publishing/12`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
request queued. So `publicationInputs` answers a theme exactly where it answers a layout.
...
Serif`, `Liberation Mono` or `STIX Two Math`) since themes 1, which set the glyph check asking a
  ...
  not, whose recorded files are not all pinned files of its family fails the publish at once,
  `typeface_unavailable` naming the family, one failure per face, rather than every character it would
  set failing `glyph_missing`.
...
- **The publication templates.** `apps/worker/templates/publication/1/main.typ` reads `assemble`'s
  ...
  `template.ts` and annotated with its literal, `'publishing/12'`, so repointing `PUBLISHING_SCHEMA`
  fails to compile there, and a frozen schema's row by its own frozen constant; `satisfies` refuses a
  row for a schema nothing makes, `publishing/9`'s, `publishing/10`'s and `publishing/11`'s excepted, which are kept. A version is immutable:
