...
else opens for reading only, and nothing pastes, recovers an iteration or
edits metadata.
...
| `editor: src/view.ts`, `style.css`          | `mountEditor`: spellcheck, language and direction on the surface, a rule per mark, and paste and drop refused; `src/render.ts`, `renderContent`: a component's content as markup through the same mapping and schema, with no view, for a document's text                                                                                 |
