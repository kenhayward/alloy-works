...
> of figures and tables after the contents, or a Word document of all of that, its footnotes and
> cross-references among it, but its equations, or both, keeps it and lists it - and [assets](#assets), which takes an
...
cross-references, designed in [`design/word-output.md`](design/word-output.md).
...
published, and every other inline item - an image, a footnote, a cross-reference and an equation - each set from the environment's theme, tables and images from their table and image styles. A Word document carries a document's paragraphs, headings, marks, links and languages, its lists, quotations, preformatted text, tables, figures and images in a line, its footnotes as Word's own and its cross-references as fields Word updates, and its page - the cover, the contents, the lists of figures and tables, the running heads and feet and the notice - and refuses equations, a reference to one and the list of equations by name (`word_not_yet`) until Word 4 writes them. A list of equations in the default layout, veraPDF on every publication, preview and the rest of Word are later slices'. Nothing chooses or edits a
...
| `domain: word/`                                            | The Word writer (below): `writeDocx`, `WORD_WRITER_VERSION` (`word/3`); `numbering.ts`, the heading numbering definitions, `captionField`, the one shape a caption's number is written in, `sequenceName`, `footnoteSections` and `footnoteProperties`, the sections Word counts footnotes in and each one's `w:footnotePr`, and `numberingNotInWord`, which follows Word's caption fields and footnote counts through the outline; `lists.ts`, a list's numbering definition, its marker's width and `listsNotInWord`; `advances.ts`, a face file's advances, `faceAdvances`; `spacing.ts`, `spacingOverrides`, what Word would read differently of the spaces the PDF wants; `panels.ts`, `panelsApart`, which panels to stand a twip further in; and `fonts.ts`, a face's key and its obfuscation                                                                                                                                                                                                                                                                                                                                                                                                |
...
- **`assemble` told its formats, and what it answers for Word** (Word 1). `AssembleInput.formats` is
  ...
  equation, and in a line or a title an equation or a cross-reference to one, and the list of
  equations - `numbering_not_in_word` a heading's, a figure's, a table's or a footnote's number
  ...
  each section title holding a reference as its words and its reference runs - exactly where Word is
  asked for, and null otherwise. The
