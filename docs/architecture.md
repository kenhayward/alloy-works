...
**Nothing publishes yet from the application.** [`design/publishing.md`](design/publishing.md)
describes the whole of it; what is here is the first of its slices, without yet the routes or the page
that would ask for a publication. The database can record a request as its publisher, and the worker
claims a `publish` job for it beside the one that renders the sample.
