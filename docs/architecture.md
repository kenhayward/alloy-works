...
> of figures and tables after the contents, or a Word document of its paragraphs, headings and page, or
> both, keeps it and lists it - and [assets](#assets), which takes an
...
which made a publication a PDF, a Word document or both, designed in
[`design/word-output.md`](design/word-output.md).
...
published, and every other inline item - an image, a footnote, a cross-reference and an equation - each set from the environment's theme, tables and images from their table and image styles. A Word document carries a document's paragraphs, headings, marks, links and languages and its page - the cover, the contents, the running heads and feet and the notice - and refuses the rest by name (`word_not_yet`) until later slices write it. A list of equations in the default layout, veraPDF on every publication, preview and the rest of Word are later slices'. Nothing chooses or edits a
...
| `domain: publishing/layout.ts`                             | The layout's stored shape at schema 5, strict at every depth, with `matter.lists` - the generated lists it declares, each sequence once, each with its title - `words.above` and `words.below`, both or neither, the words a relative cross-reference prints, and `words.continued`, required from schema 4, the words a continued table's label adds; `formats.pdf` and, from schema 5, an optional `formats.docx`, each a `PageFormat` held to the same rules, the Word page no more than 1584pt each way; `PUBLISHING_FORMATS`, `pdf` then `docx`; `layoutWordsSchema`, the words alone; `parseLayout`, `readLayout` and its migration chain, which reads a schema 1 layout as declaring no lists, a schema 2 one as having no words for above and below, one before schema 4 as having no `continued`, and one before schema 5 as having no Word page; `FIRST_DEFAULT_LAYOUT`, 0018's literal, `SECOND_DEFAULT_LAYOUT`, 0019's, `THIRD_DEFAULT_LAYOUT`, 0021's, `FOURTH_DEFAULT_LAYOUT`, 0023's, and `FIFTH_DEFAULT_LAYOUT`, 0025's, all frozen, read through today's chain when the module loads, and `defaultLayout`, 0027's; `speaksFor` (RFC 4647 basic filtering) and `unsupportedFormats` |
| `domain: publishing/outputs.ts`                            | `OUTPUT_CONTENT_TYPES`, each format's media type; an output's report, `outputReportSchema`, a list of closed entries, each once - `face_substituted` with the theme's family and the Word family it became, `no_page_cited_output` and `pages_cite_the_pdf` - and `parseOutputReport`, which the store reads and writes through                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `domain: word/`                                            | The Word writer (below): `writeDocx`, `WORD_WRITER_VERSION` (`word/1`), `numbering.ts`, the heading numbering definitions and `numberingNotInWord`, and `fonts.ts`, a face's key and its obfuscation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `domain: theme/`                                           | The theme model (see [workspaces](#workspaces)): `catalogue/2`, `catalogue/1` read by `upgradeCatalogue1`, and `theme/1`, `readCatalogue` and `readTheme`, contrast, `DEFAULT_THEME` at 0.3, `SECOND_DEFAULT_THEME` at 0.2, frozen, `DEFAULT_CATALOGUES` and `DEFAULT_CATALOGUE_VERSIONS` at 0.2, which both name, and their `FIRST_*` 0.1s, and `projectTypst`, which `assemble` puts in `publishing/13`, and `projectTypst12`, `publishing/12`'s, frozen; and the Word projection the writer includes, `projectStylesXml`, every paragraph and character style as a Word style with every property stated, and `wordRun`, a run's character style and what Word's toggling would lose pinned beside it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `db: src/publishing.ts`                                    | `resolveOccurrences` and `requestPublication`, which resolve every reference as the publisher - restricted to what they may read inside the query - and every image the resolved components place, by the same readable-set predicate, and record the request, its formats, its occurrences, its images, the declared theme's latest version and its job, refusing a request without `pdf` where anything the resolved content or a section's title holds cites a page (`page_reference.without_pdf`, PUB-074); `publicationInputs`, which returns the recorded formats and theme, `recordPublication`, which takes one output per format the request names, each with its producer and, for Word, its report, and `failPublicationRequest`, the job's; `readPublicationRequest`, `readPublication` and `listPublications`, the routes'                                                                                                                                                                                                                                                                                                                                                             |
| `api-contract: publishing.ts`                              | The four routes below and their schemas - a request's `formats`, and a publication's `outputs`, one per format, each with its standard, producer, producer version, report and links; `DocumentView.mayPublish` and `DocumentView.layout.formats`, the formats the layout makes, are in `documents.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `web: src/publishing/`                                     | `Publishing.tsx`, the panel beneath a document's outline; `PublicationPage.tsx`, a publication's own page at `#/publications/{id}`; `failures.ts`, each failure's words; `formats.ts`, a publication's formats in words and each entry of a Word output's report as a sentence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `GET /v1/publications/{id}`            | `read` on the publication      | The record - the version, the formats, the engine and the template where it has a PDF, the pipeline - and each output with its size, digest, standard, producer, producer version and report and a link signed for `DOWNLOAD_SECONDS`, which saves the file as the publication's id and `.pdf` or `.docx`, never the title, because the link's query string reaches the store's logs, and for the PDF alone a `view` link a browser shows. `404` for an id that is not a publication; `503 storage_unavailable` where the environment has no store                                                  |
...
keeps it, and one made after records 0.2; 0026 adds its 0.3 the same way.
...
- **The `publish` job.** `apps/worker/src/jobs/publish.ts` reads exactly what the request recorded
  ...
  writer over what `assemble` answers for Word and the pinned face files by hash, kept in the store as
  a `.docx` by its hash and recorded as `word/1` with the writer's report; for the PDF, it compiles the
...
- **`assemble` told its formats, and what it answers for Word** (Word 1). `AssembleInput.formats` is
  ...
  PDF's measure, a table's header spanning its body, a footnote or a reference target in a header row,
  a missing continuation label's words, and the maths tree's reasons but `unreadable`, `merror` and
  `empty` - are tagged as they are made and said only where `pdf` is asked for. Where `docx` is,
  `word_not_yet` names each construct the writer does not write yet - a list, a quotation,
  preformatted text, a table, a figure, a block equation, and in a line or a title an image, an
  equation, a footnote or a cross-reference, and each list after the contents - and
  `numbering_not_in_word` a heading number Word would compute otherwise; `format_unsupported` is
  said, not thrown, for Word under a layout with no Word page. The answer carries `word: WordInput` -
  the layout's `docx` member, the resolved theme, the scheme, and image sizes against the Word page,
  an empty map until Word 2 - exactly where Word is asked for, and null otherwise. The published
  document is the same for either format, so `publishing/13`, template 13 and `PIPELINE_VERSION` did
  not move, and the PDF made beside a Word document is byte for byte the PDF made alone.
- **The Word writer.** `packages/domain/src/word/`'s `writeDocx({ document, numbering, word, formats,
faces })`, pure and deterministic - a fixed zip time, the parts in a fixed order, relationships
